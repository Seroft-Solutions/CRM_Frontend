...
function updateFileImports(filePath) {
  try {
    ...
    const multiLinePatterns = [
      ...
      {
        regex:
          /import\s*{\s*([^}]*)\s*}\s*from\s*['"]@\/hooks\/(use-session-monitor|use-activity-tracker)['"]/g,
      ...
      {
        regex:
          /import\s*{\s*([^}]*)\s*}\s*from\s*['"]@\/lib\/(auth-utils|auth-actions|auth-token|token-refresh|token-storage|token-cache|session-events)['"]/g,
        replacement: (match, imports) => {
          return `import { ${imports.trim()} } from '@/core/auth'`;
        },
      },
      {
        regex:
          /import\s*{\s*([^}]*)\s*}\s*from\s*['"]@\/providers\/(session-provider|session-manager)['"]/g,
        replacement: (match, imports) => {
          return `import { ${imports.trim()} } from '@/core/auth'`;
        },
      },
