...
import { getEntities, searchEntities } from './city.reducer';
  ...
  return (
    <div>
      <h2 id="city-heading" data-cy="CityHeading">
        <Translate contentKey="crmBackendApp.city.home.title">Cities</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="crmBackendApp.city.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/city/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="crmBackendApp.city.home.createLabel">Create new City</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('crmBackendApp.city.home.search')}
                />
        ...
        {cityList && cityList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="crmBackendApp.city.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="crmBackendApp.city.name">Name</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                <th>
                  <Translate contentKey="crmBackendApp.city.district">District</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {cityList.map((city, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{city.name}</td>
                  <td>{city.district ? <Link to={`/district/${city.district.id}`}>{city.district.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="crmBackendApp.city.home.notFound">No Cities found</Translate>
            </div>
          )
        )}
      </div>
