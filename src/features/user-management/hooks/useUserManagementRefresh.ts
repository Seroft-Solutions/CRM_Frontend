...
import { USER_MANAGEMENT_QUERY_KEYS } from './index';

export function useUserManagementRefresh() {
  const queryClient = useQueryClient();

  const refreshOrganizationUsers = useCallback(async (organizationId: string) => {
  ...
  const refreshUserDetails = useCallback(async (organizationId: string, userId: string) => {
    await queryClient.invalidateQueries({
  ...
  const refreshAllUserData = useCallback(async (organizationId: string) => {
