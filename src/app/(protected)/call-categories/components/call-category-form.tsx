...
import { CallCategoryStepBasic } from "./steps/call-category-step-basic";
import { CallCategoryStepReview } from "./steps/call-category-step-review";
...
export function CallCategoryForm({ id }: CallCategoryFormProps) {
  ...
  // Form setup
  const form = useForm<z.infer<typeof callCategorySchema>>({
    resolver: zodResolver(callCategorySchema),
    defaultValues: {
      name: "",
      description: "",
      remark: "",
  ...
  // API hooks
  const { data: existingCallCategory } = useGetCallCategory(
    { id: id || "" },
    { query: { enabled: !isNew && !!id } }
  );

  const createCallCategoryMutation = useCreateCallCategory({
    mutation: {
      onSuccess: (data) => {
        callCategoryToast.created(data.data);
        router.push("/call-categorys");
      },
      onError: handleCallCategoryError,
    },
  });

  const updateCallCategoryMutation = useUpdateCallCategory({
    mutation: {
      onSuccess: (data) => {
        callCategoryToast.updated(data.data);
        router.push("/call-categorys");
      },
      onError: handleCallCategoryError,
    },
  ...
  // Load existing data
  if (existingCallCategory && !form.formState.isDirty) {
    const data = existingCallCategory.data;
    if (data) {
      const formData: any = {};
  ...
  // Form submission
  const onSubmit = (values: z.infer<typeof callCategorySchema>) => {
    ...
    const callCategoryData: CallCategoryDTO = {
      name: values.name,
    ...
    if (isNew) {
      createCallCategoryMutation.mutate({ data: callCategoryData });
    } else {
      updateCallCategoryMutation.mutate({
        id: id!,
        data: { ...existingCallCategory?.data, ...callCategoryData },
      });
    }
  };

  const isLoading = createCallCategoryMutation.isPending || updateCallCategoryMutation.isPending;
          ...
          <Card>
            <CardContent className="p-4 sm:p-6">
              ...
              {STEPS[currentStep].id === 'basic' && (
                <CallCategoryStepBasic form={form} />
              )}

              {STEPS[currentStep].id === 'review' && (
                <CallCategoryStepReview form={form} />
              )}
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:justify-between">
            ...
            {STEPS[currentStep].id === 'review' && confirmSubmission ? (
              ...
              >
                ...
                {isLoading ? "Saving..." : `${isNew ? "Create" : "Update"} CallCategory`}
              </Button>
            ) : STEPS[currentStep].id === 'review' ? (
              <Button
                type="submit"
...
export default CallCategoryForm;