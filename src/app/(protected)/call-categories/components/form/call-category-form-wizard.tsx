...
function CallCategoryFormContent({ id }: CallCategoryFormProps) {
  ...
  const { mutate: createEntity, isPending: isCreating } = useCreateCallCategory({
    mutation: {
      onSuccess: (data) => {
        ...
        console.log('🟢 CallCategory created successfully with ID:', entityId);
        
        if (hasReferrer() && entityId) {
          console.log('🟢 Has referrer, navigating back with entity ID:', entityId);
          // Don't show toast here - success will be shown on the referring form
          setIsRedirecting(true);
          navigateBackToReferrer(entityId, 'CallCategory');
        } else {
          console.log('🟢 No referrer, going to call-categories list');
          setIsRedirecting(true);
      ...
      onError: (error) => {
        console.error('🔴 CallCategory creation failed:', error);
