...
export function GroupForm({ id }: GroupFormProps) {
  ...
  const [formSessionId] = useState(() => {
    ...
    const existingSession = sessionStorage.getItem('Group_FormSession');
    if (existingSession && isNew) {
      return existingSession;
    }
    const newSessionId = `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    if (isNew) {
      sessionStorage.setItem('Group_FormSession', newSessionId);
    }
    return newSessionId;
  });
  ...
  const saveFormState = React.useCallback(() => {
    ...
    const storageKey = `GroupFormState_${formSessionId}`;
  ...
  const restoreFormState = React.useCallback(() => {
    ...
    if (entityCreationContext) {
      try {
        ...
        if (context.sourceEntity && context.sourceEntity !== 'Group') {
    ...
    const currentSessionId = sessionStorage.getItem('Group_FormSession');
    ...
    if (savedStateStr) {
      try {
        ...
        const isSameEntity = savedState.entity === 'Group';
  ...
  const clearOldFormStates = React.useCallback(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith('GroupFormState_') && !key.endsWith(formSessionId)) {
  ...
  useEffect(() => {
    if (entity && !isRestoring) {
      const formValues = {
        ...
        keycloakGroupId: entity.keycloakGroupId || "",
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    const entityToSave = {
      ...
      keycloakGroupId: data.keycloakGroupId === "__none__" ? undefined : data.keycloakGroupId,


      name: data.name === "__none__" ? undefined : data.name,


      path: data.path === "__none__" ? undefined : data.path,
  ...
  const cleanupFormState = React.useCallback(() => {
    const storageKey = `GroupFormState_${formSessionId}`;
    localStorage.removeItem(storageKey);
    sessionStorage.removeItem('Group_FormSession');
    
    // Clear all old form states for this entity type
    const keysToRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key?.startsWith('GroupFormState_')) {
        keysToRemove.push(key);
      }
    }
    keysToRemove.forEach(key => localStorage.removeItem(key));
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <PaginatedRelationshipCombobox
                              ...
                              onEntityCreated={(entityId) => handleEntityCreated(entityId, 'members')}
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                      <div className="space-y-1">
                        <dt className="text-sm font-medium text-muted-foreground">Keycloak Group Id</dt>
                        <dd className="text-sm">
                          {form.watch('keycloakGroupId') || "—"}
                        </dd>
                      </div>
                      <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                        ...
                        <dd className="text-sm">
                          <Badge variant="outline">
                            {Array.isArray(form.watch('members')) ? 
                              `${form.watch('members').length} selected` : 'None selected'}
                          </Badge>
                        </dd>
                      </div>
                    </div>
            ...
            {currentStep === STEPS.length - 1 ? (
              !confirmSubmission ? (
                ...
                >
                  ...
                  {isCreating || isUpdating ? "Submitting..." : `${isNew ? "Create" : "Update"} Group`}
                </Button>
              )
            ) : (
              <Button
