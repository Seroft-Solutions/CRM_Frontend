...
export function UserAvailabilityTable() {
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `user-availability-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': UserAvailabilityDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': UserAvailabilityDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': UserAvailabilityDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': UserAvailabilityDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': UserAvailabilityDTOStatus.ACTIVE };
            ...
            (old: any[]) =>
              old.map((userAvailability) =>
                userAvailability.id === variables.id
                  ? { ...userAvailability, ...variables.data }
                  : userAvailability
              )
          );
        }
    ...
    mutation: {
      ...
      onSuccess: (data, variables) => {
        ...
        if (currentStatusFilter !== newStatus && activeStatusTab !== 'all') {
          ...
          queryClient.setQueryData(['countUserAvailabilities', filterParams], (old: number) =>
            Math.max(0, (old || 0) - 1)
          );
        }
      },
    ...
    <>
      ...
      <div className="w-full space-y-4">
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected user availabilities to "Archived". They
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected user availabilities to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Archive this useravailability?</AlertDialogTitle>
              <AlertDialogDescription>
                This will change the status to "Archived". The useravailability will no longer
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this useravailability to "
