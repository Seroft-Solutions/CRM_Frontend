...
export function CustomerDataImport() {
  ...
  const handleSubmit = (data: any) => {
    ...
    importCustomers({ data: { file: data.importFile } });
  };
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      link.href = url;
      link.download = customerImportConfig.filename;
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        <Card>
          <CardContent className="p-4 sm:p-6">
            <h4 className="font-semibold mb-2">Customer Data Bulk Import Instructions</h4>
            <div className="space-y-4">
              {/* Instructions List */}
              <div>
                <h5 className="text-sm font-medium text-muted-foreground mb-2">Instructions:</h5>
                <ul className="list-disc pl-5 text-sm text-foreground">
                  {customerImportConfig.instructions.map((instruction, index) => (
              ...
              <div>
                ...
                <p className="text-sm text-foreground">{customerImportConfig.filename}</p>
              ...
              <div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full border-collapse border border-border">
                    ...
                    <tbody>
                      {customerImportConfig.columns.map((column, index) => (
