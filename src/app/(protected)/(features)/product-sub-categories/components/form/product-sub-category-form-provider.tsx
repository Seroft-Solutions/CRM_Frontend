...
interface ProductSubCategoryFormProviderProps {
...
export function ProductSubCategoryFormProvider({
  children,
  id,
  onSuccess,
  onError,
}: ProductSubCategoryFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = productSubCategoryFormConfig;
  ...
  const form = useForm<Record<string, any>>({
    resolver: zodResolver(productSubCategoryFormSchema),
      ...
      if (savedStateStr) {
        try {
          ...
          if (isRecent && isSameSession && isSameEntity && isCrossFormState) {
            ...
            if (!suppressToast) {
              productSubCategoryToast.formRestored();
            }
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a ProductSubCategoryFormProvider');
  }
  return context;
}
