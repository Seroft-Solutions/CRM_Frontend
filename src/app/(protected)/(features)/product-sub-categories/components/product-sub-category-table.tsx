...
import {
  ...
  productSubCategoryToast} from './product-sub-category-toast';
...
import {
  ...
  useUpdateProductSubCategory} from '@/core/api/generated/spring/endpoints/product-sub-category-resource/product-sub-category-resource.gen';
...
import { ProductSubCategoryTableHeader } from './table/product-sub-category-table-header';
import { ProductSubCategoryTableRow } from './table/product-sub-category-table-row';
...
const COLUMN_VISIBILITY_KEY = 'product-sub-category-table-columns';
...
export function ProductSubCategoryTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'product-sub-category-table-columns';
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `product-sub-category-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const statusOptions = [
    {
      value: ProductSubCategoryDTOStatus.DRAFT,
      label: transformEnumValue('DRAFT'),
      color: 'bg-gray-100 text-gray-800'},
    {
      value: ProductSubCategoryDTOStatus.ACTIVE,
      label: transformEnumValue('ACTIVE'),
      color: 'bg-green-100 text-green-800'},
    {
      value: ProductSubCategoryDTOStatus.INACTIVE,
      label: transformEnumValue('INACTIVE'),
      color: 'bg-yellow-100 text-yellow-800'},
    {
      value: ProductSubCategoryDTOStatus.ARCHIVED,
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': ProductSubCategoryDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': ProductSubCategoryDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': ProductSubCategoryDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': ProductSubCategoryDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': ProductSubCategoryDTOStatus.ACTIVE };
  ...
  const { data: countData } = useCountProductSubCategories(filterParams, {
    query: {
      enabled: true,
      staleTime: 0,
      refetchOnWindowFocus: true}});
    ...
    mutation: {
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) =>
            old?.map((productSubCategory) =>
              productSubCategory.id === variables.id ? data : productSubCategory
            )
        );
    ...
    mutation: {
      ...
      onSuccess: (data, variables) => {
        ...
        productSubCategoryToast.custom.success(
          `Status Updated`,
          `ProductSubCategory status changed to ${statusLabel}`
        );
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: ProductSubCategoryDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue =
        ProductSubCategoryDTOStatus[bulkNewStatus as keyof typeof ProductSubCategoryDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
  ...
  const handleRelationshipUpdate = async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (!currentEntity) {
        ...
        reject(new Error('ProductSubCategory not found in current data'));
        return;
      }
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) =>
                  old?.map((productSubCategory) =>
                    productSubCategory.id === entityId ? serverResponse : productSubCategory
                  )
              );
            ...
            if (!isBulkOperation) {
              productSubCategoryToast.relationshipUpdated(relationshipName);
            }
            resolve();
          },
          onError: (error: any) => {
    ...
    <>
      ...
      <div className="w-full space-y-4">
            ...
            <Table className="w-full">
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  data.map((productSubCategory) => (
                    <ProductSubCategoryTableRow
                      ...
                      isSelected={selectedRows.has(productSubCategory.id || 0)}
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected product sub categories to "Archived".
                They will no longer appear in the active view but can be restored later.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected product sub categories to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Archive this productsubcategory?</AlertDialogTitle>
              <AlertDialogDescription>
                This will change the status to "Archived". The productsubcategory will no longer
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this productsubcategory to "
