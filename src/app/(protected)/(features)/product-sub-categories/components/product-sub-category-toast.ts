...
// Toast notification utilities for ProductSubCategory
export const productSubCategoryToast = {
  // Success messages
  created: (entityName?: string) =>
    toast.success('✅ Success!', {
      description: `${entityName || 'Product Sub Category'} created successfully`,
      action: {
        label: 'View',
        onClick: () => (window.location.href = '/product-sub-categories'),
      },
    }),

  updated: (entityName?: string) =>
    toast.success('✅ Updated!', {
      description: `${entityName || 'Product Sub Category'} updated successfully`,
      action: {
        label: 'View All',
        onClick: () => (window.location.href = '/product-sub-categories'),
      },
  ...
  deleted: (entityName?: string) =>
    toast.success('🗑️ Deleted!', {
      description: `${entityName || 'Product Sub Category'} deleted successfully`,
    }),

  bulkDeleted: (count: number) =>
    toast.success('🗑️ Bulk Delete!', {
      description: `${count} product sub categories deleted successfully`,
    }),
  ...
  exported: (format: string) =>
    toast.success('📤 Exported!', {
      description: `Product Sub Categories exported to ${format} successfully`,
    }),

  imported: (count: number) =>
    toast.success('📥 Imported!', {
      description: `${count} product sub categories imported successfully`,
    }),

  // Error messages
  createError: (error?: string) =>
    toast.error('❌ Creation Failed', {
      description: error || `Failed to create productsubcategory. Please try again.`,
      action: {
        label: 'Retry',
  ...
  updateError: (error?: string) =>
    toast.error('❌ Update Failed', {
      description: error || `Failed to update productsubcategory. Please try again.`,
      action: {
        label: 'Retry',
        onClick: () => window.location.reload(),
      },
  ...
  deleteError: (error?: string) =>
    toast.error('❌ Delete Failed', {
      description: error || `Failed to delete productsubcategory. Please try again.`,
    }),

  bulkDeleteError: (error?: string) =>
    toast.error('❌ Bulk Delete Failed', {
      description: error || `Failed to delete selected product sub categories. Please try again.`,
    }),
...
export const handleProductSubCategoryError = (error: any) => {
  const errorMessage =
    error?.response?.data?.message || error?.message || 'An unexpected error occurred';

  if (error?.response?.status === 403) {
    productSubCategoryToast.permissionError();
  } else if (error?.response?.status === 422) {
    const validationErrors = error?.response?.data?.fieldErrors;
    const fields = validationErrors ? Object.keys(validationErrors) : [];
    productSubCategoryToast.validationError(fields);
  } else if (error?.code === 'NETWORK_ERROR') {
    productSubCategoryToast.networkError();
  } else {
    toast.error('❌ Error', {
