...
export const channelTypeFormSchemaFields = {
  ...
  commissionRate: z.string().refine(val => !val || Number(val) >= 0, { message: "Please enter a number 0 or higher" }).refine(val => !val || Number(val) <= 100, { message: "Please enter a number 100 or lower" }).optional(),
};
