...
interface ChannelTypeFormProviderProps {
...
export function ChannelTypeFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: ChannelTypeFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = channelTypeFormConfig;
  ...
  const form = useForm<Record<string, any>>({
    resolver: zodResolver(channelTypeFormSchema),
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a ChannelTypeFormProvider');
  }
  return context;
}
