...
import { AdvancedPagination, usePaginationState } from "@/app/(protected)/(features)/sources/components/table/advanced-pagination";
...
export function SourceTable() {
  ...
  const { data: countData } = useCountSources(
    filterParams,
    {
      query: {
        enabled: true,
  ...
  const { mutate: updateEntity, isPending: isUpdating } = useUpdateSource({
    mutation: {
      onMutate: async (variables) => {
        ...
        const previousData = queryClient.getQueryData(['getAllSources', {
          page: apiPage,
          size: pageSize,
          sort: [`${sort},${order}`],
          ...filterParams,
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(['getAllSources', {
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
            ...filterParams,
        ...
        if (searchTerm) {
          queryClient.setQueryData(['searchSources', {
            query: searchTerm,
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
  ...
  const { mutate: deleteEntity, isPending: isDeleting } = useDeleteSource({
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(['getAllSources', {
          ...
          old?.filter(source => source.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(['getAllSources', {
        ...
        old?.filter(source => !selectedRows.has(source.id || 0))
      );
      ...
      sourceToast.bulkDeleted(selectedRows.size);
      setSelectedRows(new Set());
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              selected sources and remove their data from the server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the
              source and remove its data from the server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
