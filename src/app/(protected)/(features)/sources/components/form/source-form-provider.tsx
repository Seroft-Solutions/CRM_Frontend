...
import { sourceFormSchema } from '@/app/(protected)/(features)/sources/components/form/source-form-schema';
import {
  sourceToast,
  handleSourceError,
...
interface SourceFormProviderProps {
...
export function SourceFormProvider({ children, id, onSuccess, onError }: SourceFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = sourceFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a SourceFormProvider');
  }
  return context;
}
