  ...
  return (
    <TableRow className="hover:bg-gray-50 transition-colors">
      <TableCell className="w-10 sm:w-12 px-2 sm:px-3 py-2 sticky left-0 bg-white z-10">
        <Checkbox checked={isSelected} onCheckedChange={() => source.id && onSelect(source.id)} />
      </TableCell>
          ...
          <InlinePermissionGuard requiredPermission="source:update">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {currentStatus !== SourceDTOStatus.ACTIVE && (
                  <DropdownMenuItem
                    onClick={() => source.id && onStatusChange(source.id, 'ACTIVE')}
                ...
                {currentStatus !== SourceDTOStatus.INACTIVE && (
                  <DropdownMenuItem
                    onClick={() => source.id && onStatusChange(source.id, 'INACTIVE')}
                ...
                {currentStatus !== SourceDTOStatus.DRAFT && (
                  <DropdownMenuItem
                    onClick={() => source.id && onStatusChange(source.id, 'DRAFT')}
                ...
                {currentStatus !== SourceDTOStatus.ARCHIVED && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onClick={() => source.id && onArchive(source.id)}
