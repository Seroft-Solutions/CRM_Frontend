...
function RelationshipDisplayValue({ 
  ...
  // Get the appropriate hook for this relationship
    const { data: meetingData } = relConfig.name === 'meeting' ? 
    useGetAllMeetings({ page: 0, size: 1000 }, {
      query: {
        enabled: !!value && relConfig.name === 'meeting',
        staleTime: 5 * 60 * 1000,
      }
    }) : { data: null };
...
export function MeetingParticipantDetails({ id }: MeetingParticipantDetailsProps) {
  const router = useRouter();
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  // Get form config for step organization
  const formConfig = meetingParticipantFormConfig;

  // Fetch entity details
  const { data: entity, isLoading } = useGetMeetingParticipant(id, {
    query: {
      enabled: !!id,
    },
  });
  ...
  // Delete mutation
  const { mutate: deleteEntity } = useDeleteMeetingParticipant({
    mutation: {
      onSuccess: () => {
      ...
      <div className="mt-8 pt-6 border-t">
        <div className="flex flex-col sm:flex-row gap-3 sm:justify-end">
          <Button variant="outline" asChild className="flex items-center gap-2 justify-center">
            <Link href={`/meeting-participants/${id}/edit`}>
