...
import { SundryCreditorTableHeader } from './table/sundry-creditor-table-header';
import { SundryCreditorTableRow } from './table/sundry-creditor-table-row';
...
export function SundryCreditorTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'sundry-creditor-table-columns';
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `sundry-creditor-export-${new Date().toISOString().split('T')[0]}.csv`;
      ...
      if (value !== undefined && value !== '' && value !== null) {
        if (relationshipMappings[key]) {
          ...
          if (typeof value === 'string' && value.trim() !== '') {
            params['creditorName.contains'] = value;
          }
        } else if (key === 'email') {
          if (typeof value === 'string' && value.trim() !== '') {
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateSundryCreditor({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllSundryCreditors'] });
      ...
      onSuccess: (data, variables) => {
        ...
        if (currentStatusFilter !== newStatus && activeStatusTab !== 'all') {
          ...
          queryClient.setQueryData(['countSundryCreditors', filterParams], (old: number) =>
            Math.max(0, (old || 0) - 1)
          );
        }
      },
  ...
  const confirmBulkArchive = async () => {
    ...
    try {
      ...
      sundryCreditorToast.custom.error(
        'Bulk Archive Failed',
  ...
  const handleRelationshipUpdate = async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (!currentEntity) {
        ...
        reject(new Error('Sundry creditor not found in current data'));
        return;
      }
      ...
      updateEntity(
        ...
        },
        {
          onSuccess: (serverResponse) => {
            ...
            if (!isBulkOperation) {
              sundryCreditorToast.relationshipUpdated(relationshipName);
            }
            resolve();
          },
          onError: (error: any) => {
  ...
  const handleBulkRelationshipUpdate = async (
    ...
    try {
      ...
      if (errorCount === entityIds.length) {
        ...
        sundryCreditorToast.custom.warning(
          '⚠️ Partial Success',
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected sundry creditors to "Archived". They
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected sundry creditors to "
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this sundry creditor to "
