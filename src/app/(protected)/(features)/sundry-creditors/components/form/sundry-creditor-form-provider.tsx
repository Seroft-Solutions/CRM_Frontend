...
import { sundryCreditorFormConfig } from './sundry-creditor-form-config';
import { sundryCreditorFormSchema } from './sundry-creditor-form-schema';
...
interface SundryCreditorFormProviderProps {
...
export function SundryCreditorFormProvider({
  ...
  const isNew = !id;
  const config = sundryCreditorFormConfig;
      ...
      if (savedStateStr) {
        try {
          ...
          if (isRecent && isSameSession && isSameEntity && isCrossFormState) {
            ...
            if (!suppressToast) {
              sundryCreditorToast.formRestored();
            }
  ...
  function transformFormDataForSubmission(data: Record<string, any>) {
    ...
    config.relationships.forEach((relConfig) => {
      ...
      if (relConfig.multiple) {
        ...
        if (value) {
          const idValue =
            typeof value === 'object' && value !== null ? value[relConfig.primaryKey] : value;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a SundryCreditorFormProvider');
  }
  return context;
}
