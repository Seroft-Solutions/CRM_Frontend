...
/**
 * Configuration for Sundry Creditor form
 */
export const sundryCreditorFormConfig: FormConfig = {
  entity: 'SundryCreditor',
  ...
  fields: [
    {
      ...
      type: 'text',
      label: 'Creditor Name',
      placeholder: 'Enter creditor name (minimum 2 characters)',
      required: true,
      validation: {
        required: true,
        minLength: 2,
  ...
  behavior: {
    ...
    persistence: {
      ...
      storagePrefix: 'SundryCreditorFormState_',
    },
    navigation: {
...
export const sundryCreditorFormHelpers = {
  getStepById: (stepId: string) => sundryCreditorFormConfig.steps.find((step) => step.id === stepId),
  getFieldConfig: (fieldName: string) =>
    sundryCreditorFormConfig.fields.find((field) => field.name === fieldName),
  getRelationshipConfig: (relationshipName: string) =>
    sundryCreditorFormConfig.relationships.find((rel) => rel.name === relationshipName),
  getStepFields: (stepId: string) => {
    const step = sundryCreditorFormConfig.steps.find((s) => s.id === stepId);
