...
export function SundryCreditorDetails({ id }: SundryCreditorDetailsProps) {
  const router = useRouter();
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  const formConfig = sundryCreditorFormConfig;

  const { data: entity, isLoading } = useGetSundryCreditor(id, {
    enabled: !!id,
  });

  const { mutate: deleteEntity } = useDeleteSundryCreditor({
    onSuccess: () => {
      sundryCreditorToast.deleted();
