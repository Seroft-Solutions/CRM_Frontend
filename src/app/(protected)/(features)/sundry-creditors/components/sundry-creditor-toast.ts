...
export const sundryCreditorToast = {
    created: (entityName?: string) =>
        toast.success('✅ Success!', {
            description: `${entityName || 'Sundry Creditor'} created successfully`,
            action: {
                label: 'View',
                onClick: () => (window.location.href = '/sundry-creditors'),
            },
        }),

    updated: (entityName?: string) =>
        toast.success('✅ Updated!', {
            description: `${entityName || 'Sundry Creditor'} updated successfully`,
            action: {
                label: 'View All',
                onClick: () => (window.location.href = '/sundry-creditors'),
            },
    ...
    deleted: (entityName?: string) =>
        toast.success('🗑️ Deleted!', {
            description: `${entityName || 'Sundry Creditor'} deleted successfully`,
        }),

    bulkDeleted: (count: number) =>
        toast.success('🗑️ Bulk Delete!', {
            description: `${count} sundry creditors deleted successfully`,
        }),
    ...
    exported: (format: string) =>
        toast.success('📤 Exported!', {
            description: `Sundry Creditors exported to ${format} successfully`,
        }),

    imported: (count: number) =>
        toast.success('📥 Imported!', {
            description: `${count} sundry creditors imported successfully`,
        }),

    createError: (error?: string) =>
        toast.error('❌ Creation Failed', {
            description: error || `Failed to create sundry creditor. Please try again.`,
            action: {
                label: 'Retry',
    ...
    updateError: (error?: string) =>
        toast.error('❌ Update Failed', {
            description: error || `Failed to update sundry creditor. Please try again.`,
            action: {
                label: 'Retry',
                onClick: () => window.location.reload(),
            },
    ...
    deleteError: (error?: string) =>
        toast.error('❌ Delete Failed', {
            description: error || `Failed to delete sundry creditor. Please try again.`,
        }),

    bulkDeleteError: (error?: string) =>
        toast.error('❌ Bulk Delete Failed', {
            description: error || `Failed to delete selected sundry creditors. Please try again.`,
        }),
...
export const handleSundryCreditorError = (error: any) => {
    const errorMessage =
        error?.response?.data?.message || error?.message || 'An unexpected error occurred';

    if (error?.response?.status === 403) {
        sundryCreditorToast.permissionError();
    } else if (error?.response?.status === 422) {
        const validationErrors = error?.response?.data?.fieldErrors;
        const fields = validationErrors ? Object.keys(validationErrors) : [];
        sundryCreditorToast.validationError(fields);
    } else if (error?.code === 'NETWORK_ERROR') {
        sundryCreditorToast.networkError();
    } else {
        toast.error('❌ Error', {
