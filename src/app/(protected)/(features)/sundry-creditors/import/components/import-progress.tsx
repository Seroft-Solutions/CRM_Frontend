...
import { useGetSundryCreditorImportProgress } from '../../api/sundry-creditor';
...
export function ImportProgress({ jobId, onComplete }: ImportProgressProps) {
  ...
  const { data: progress, isLoading } = useGetSundryCreditorImportProgress(jobId, {
  ...
  useEffect(() => {
    if (progress?.status === 'COMPLETED' && !hasNavigated.current) {
      ...
      if (progress.result) {
        sessionStorage.setItem('sundryCreditorImportResponse', JSON.stringify(progress.result));
      }
      ...
      setTimeout(() => {
        router.push('/sundry-creditors/import/results');
      }, 1500);
    }
