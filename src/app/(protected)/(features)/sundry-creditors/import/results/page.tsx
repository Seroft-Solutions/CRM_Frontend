...
import { FailedSundryCreditorsTable } from '../components/failed-sundry-creditors-table';
...
export default function ImportResultsPage() {
  ...
  useEffect(() => {
    const storedData = sessionStorage.getItem('sundryCreditorImportResponse');

    if (storedData) {
      try {
        const data = JSON.parse(storedData);
        setResponseData(data);
        sessionStorage.removeItem('sundryCreditorImportResponse');
      } catch (error) {
  ...
  const handleDownloadReport = () => {
    ...
    const headers = [
      'Row #',
      ...sundryCreditorImportConfig.columns.map((c) => c.header),
      'Status',
      'Reason',
    ];
    const sheetRows = tableRows.map((row) => [
      row.rowNumber,
      ...sundryCreditorImportConfig.columns.map((_, idx) => row.values[idx] ?? ''),
      row.status,
      row.reason,
    ]);
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Sundry Creditor Import Results');
    XLSX.writeFile(workbook, 'sundry-creditor-import-results.xlsx');
  };
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button asChild>
              <Link href="/sundry-creditors/import">Start New Import</Link>
            </Button>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild>
              <Link href="/sundry-creditors/import">Import Another File</Link>
            </Button>
          </div>
        </CardHeader>
