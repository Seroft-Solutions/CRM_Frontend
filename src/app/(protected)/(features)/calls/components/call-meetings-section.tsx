...
export function CallMeetingsSection({ callId, customerId, assignedUserId }: CallMeetingsSectionProps) {
  ...
  const [sorting, setSorting] = useState<SortingState>([
    { id: "meetingDateTime", desc: false }
  ]);
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [globalFilter, setGlobalFilter] = useState("");

  // Fetch meetings for this specific call
  const { data: meetings = [], isLoading, refetch } = useGetAllMeetings(
    {
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (!organizer) return <span className="text-muted-foreground text-xs">No organizer</span>;
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const meeting = row.original;
          const canCancel = meeting.meetingStatus === MeetingDTOMeetingStatus.SCHEDULED || 
            ...
            <div className="space-y-6">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm">
                    {selectedMeeting.meetingDateTime && 
                      format(new Date(selectedMeeting.meetingDateTime), "EEEE, MMMM d, yyyy 'at' h:mm a")
                    }
                  </p>
                </div>
                <div>
              ...
              {selectedMeeting.organizer && (
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <User className="h-4 w-4 text-muted-foreground" />
                    <span className="text-sm">{selectedMeeting.organizer.displayName || selectedMeeting.organizer.firstName}</span>
                  </div>
                </div>
              )}
      ...
      <AlertDialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to cancel this meeting? This action will notify all participants.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
