...
import { RelationshipCell } from '@/app/(protected)/(features)/user-drafts/components/table/relationship-cell';
    ...
    <TableRow className="hover:bg-gray-50 transition-colors">
      ...
      <TableCell className="sticky right-0 bg-white px-2 sm:px-3 py-2 border-l border-gray-200 z-10 w-[100px] sm:w-[120px]">
        <div className="flex items-center gap-0.5 sm:gap-1">
          ...
          <InlinePermissionGuard requiredPermission="userDraft:delete">
            <Button
              variant="ghost"
              size="sm"
              className="h-6 w-6 sm:h-7 sm:w-7 p-0 text-destructive"
              onClick={() => userDraft.id && onDelete(userDraft.id)}
              disabled={isDeleting || !userDraft.id}
            >
