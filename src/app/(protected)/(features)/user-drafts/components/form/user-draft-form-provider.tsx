...
interface UserDraftFormProviderProps {
...
export function UserDraftFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: UserDraftFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = userDraftFormConfig;
  ...
  const form = useForm<Record<string, any>>({
    resolver: zodResolver(userDraftFormSchema),
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a UserDraftFormProvider');
  }
  return context;
}
