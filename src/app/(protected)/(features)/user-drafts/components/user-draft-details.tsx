...
export function UserDraftDetails({ id }: UserDraftDetailsProps) {
  const router = useRouter();
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  // Get form config for step organization
  const formConfig = userDraftFormConfig;

  // Fetch entity details
  const { data: entity, isLoading } = useGetUserDraft(id, {
    query: {
      enabled: !!id,
    },
  });
  ...
  // Delete mutation
  const { mutate: deleteEntity } = useDeleteUserDraft({
    mutation: {
      onSuccess: () => {
