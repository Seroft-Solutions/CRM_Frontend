...
// Toast notification utilities for UserProfile
export const userProfileToast = {
  // Success messages
  created: (entityName?: string) => 
    toast.success("✅ Success!", {
      description: `${entityName || 'User Profile'} created successfully`,
      action: {
        label: "View",
  ...
  updated: (entityName?: string) => 
    toast.success("✅ Updated!", {
      description: `${entityName || 'User Profile'} updated successfully`,
      action: {
        label: "View All",
        onClick: () => window.location.href = "/user-profiles"
      }
  ...
  deleted: (entityName?: string) => 
    toast.success("🗑️ Deleted!", {
      description: `${entityName || 'User Profile'} deleted successfully`
    }),
    
  bulkDeleted: (count: number) => 
    toast.success("🗑️ Bulk Delete!", {
      description: `${count} user profiles deleted successfully`
    }),
  ...
  exported: (format: string) => 
    toast.success("📤 Exported!", {
      description: `User Profiles exported to ${format} successfully`
    }),
    
  imported: (count: number) => 
    toast.success("📥 Imported!", {
      description: `${count} user profiles imported successfully`
    }),

  // Error messages
  createError: (error?: string) => 
    toast.error("❌ Creation Failed", {
      description: error || `Failed to create userprofile. Please try again.`,
      action: {
        label: "Retry",
  ...
  updateError: (error?: string) => 
    toast.error("❌ Update Failed", {
      description: error || `Failed to update userprofile. Please try again.`,
      action: {
        label: "Retry",
        onClick: () => window.location.reload()
      }
  ...
  deleteError: (error?: string) => 
    toast.error("❌ Delete Failed", {
      description: error || `Failed to delete userprofile. Please try again.`
    }),
    
  bulkDeleteError: (error?: string) => 
    toast.error("❌ Bulk Delete Failed", {
      description: error || `Failed to delete selected user profiles. Please try again.`
    }),
...
export const handleUserProfileError = (error: any) => {
  const errorMessage = error?.response?.data?.message || 
                      error?.message || 
                      'An unexpected error occurred';
  
  if (error?.response?.status === 403) {
    userProfileToast.permissionError();
  } else if (error?.response?.status === 422) {
    const validationErrors = error?.response?.data?.fieldErrors;
    const fields = validationErrors ? Object.keys(validationErrors) : [];
    userProfileToast.validationError(fields);
  } else if (error?.code === 'NETWORK_ERROR') {
    userProfileToast.networkError();
  } else {
    toast.error("❌ Error", {
