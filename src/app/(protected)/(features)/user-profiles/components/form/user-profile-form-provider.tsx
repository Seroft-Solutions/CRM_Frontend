...
import { userProfileFormSchema } from '@/app/(protected)/(features)/user-profiles/components/form/user-profile-form-schema';
import {
  userProfileToast,
...
export function UserProfileFormProvider({
  ...
  const isNew = !id;
  const config = userProfileFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a UserProfileFormProvider');
  }
  return context;
}
