...
export function GroupTableRow({
  ...
  return (
    <TableRow>
      ...
      <TableCell className="whitespace-nowrap px-3 py-2">{group.isActive ? 'Yes' : 'No'}</TableCell>
      ...
      <TableCell className="whitespace-nowrap px-1 py-2">
        <RelationshipCell
          ...
          options={
            relationshipConfigs.find((config) => config.name === 'organization')?.options || []
          }
          displayField="name"
          onUpdate={onRelationshipUpdate || (() => Promise.resolve())}
          isEditable={
            relationshipConfigs.find((config) => config.name === 'organization')?.isEditable ||
            false
          }
