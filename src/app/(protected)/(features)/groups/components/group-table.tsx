...
import {
  ...
  useGetAllGroups,
  useSearchGroups,
  useUpdateGroup} from '@/core/api/generated/spring/endpoints/group-resource/group-resource.gen';
...
export function GroupTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'group-table-columns';
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `group-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': GroupDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': GroupDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': GroupDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': GroupDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': GroupDTOStatus.ACTIVE };
    ...
    mutation: {
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.map((group) => (group.id === variables.id ? data : group))
        );
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateGroup({
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => {
            ...
            if (currentStatusFilter === newStatus || activeStatusTab === 'all') {
              ...
              return old.filter((group) => group.id !== variables.id);
            }
          }
        );
      ...
      onSuccess: (data, variables) => {
        ...
        groupToast.custom.success(`Status Updated`, `Group status changed to ${statusLabel}`);
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: GroupDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue = GroupDTOStatus[bulkNewStatus as keyof typeof GroupDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) =>
                  old?.map((group) => (group.id === entityId ? serverResponse : group))
              );
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected groups to "Archived". They will no
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected groups to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status to "Archived". The group will no longer appear in the
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this group to "
