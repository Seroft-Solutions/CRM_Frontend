...
interface PriorityFormProviderProps {
...
export function PriorityFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: PriorityFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = priorityFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a PriorityFormProvider');
  }
  return context;
}
