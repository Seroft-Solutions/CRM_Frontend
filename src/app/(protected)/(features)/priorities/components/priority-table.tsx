...
import { AdvancedPagination, usePaginationState } from "@/app/(protected)/(features)/priorities/components/table/advanced-pagination";
  ...
  const { mutate: updateEntity, isPending: isUpdating } = useUpdatePriority({
    mutation: {
      onMutate: async (variables) => {
        ...
        const previousData = queryClient.getQueryData(['getAllPriorities', {
          page: apiPage,
          size: pageSize,
          sort: [`${sort},${order}`],
          ...filterParams,
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(['getAllPriorities', {
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
            ...filterParams,
        ...
        if (searchTerm) {
          queryClient.setQueryData(['searchPriorities', {
            query: searchTerm,
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(['getAllPriorities', {
          ...
          old?.filter(priority => priority.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(['getAllPriorities', {
        ...
        old?.filter(priority => !selectedRows.has(priority.id || 0))
      );
