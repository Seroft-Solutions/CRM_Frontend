...
import { callCategoryFormConfig } from "./call-category-form-config";
import { callCategoryFormSchema } from "./call-category-form-schema";
...
interface CallCategoryFormProviderProps {
...
export function CallCategoryFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: CallCategoryFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = callCategoryFormConfig;
  ...
  const form = useForm<Record<string, any>>({
    resolver: zodResolver(callCategoryFormSchema),
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a CallCategoryFormProvider');
  }
  return context;
}
