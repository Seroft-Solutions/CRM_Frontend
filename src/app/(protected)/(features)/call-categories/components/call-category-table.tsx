...
const COLUMN_VISIBILITY_KEY = 'call-category-table-columns'; // v2 to force reset for auditing fields
...
export function CallCategoryTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'call-category-table-columns'; // Old key without version
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `call-category-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const { data: countData } = useCountCallCategories(
    filterParams,
    {
      query: {
        enabled: true,
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        await queryClient.cancelQueries({ 
          queryKey: ['getAllCallCategories'] 
        });
        
        await queryClient.cancelQueries({ 
          queryKey: ['searchCallCategories'] 
        });
        

        // Snapshot the previous value
        const previousData = queryClient.getQueryData(['getAllCallCategories', {
          page: apiPage,
          size: pageSize,
          sort: [`${sort},${order}`],
          ...filterParams,
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(['getAllCallCategories', {
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
            ...filterParams,
          }], (old: any[]) => 
            old.map(callCategory => 
              callCategory.id === variables.id 
                ? { ...callCategory, ...variables.data }
                : callCategory
            )
          );
        }
        ...
        if (searchTerm) {
          queryClient.setQueryData(['searchCallCategories', {
            query: searchTerm,
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
  ...
  const { mutate: deleteEntity, isPending: isDeleting } = useDeleteCallCategory({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllCallCategories'] });
        ...
        queryClient.setQueryData(['getAllCallCategories', {
          ...
          old?.filter(callCategory => callCategory.id !== variables.id)
        );
      ...
      onSuccess: () => {
        ...
        // Update count cache
        queryClient.setQueryData(['countCallCategories', filterParams], (old: number) => 
          Math.max(0, (old || 0) - 1)
        );
      },
      onError: (error, variables, context) => {
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(['getAllCallCategories', {
        ...
        old?.filter(callCategory => !selectedRows.has(callCategory.id || 0))
      );
      ...
      callCategoryToast.bulkDeleted(selectedRows.size);
      setSelectedRows(new Set());
  ...
  const handleRelationshipUpdate = async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (!currentEntity) {
        ...
        reject(new Error('CallCategory not found in current data'));
        return;
      }
      ...
      updateEntity({ 
        ...
        onSuccess: (serverResponse) => {
          ...
          if (!isBulkOperation) {
            callCategoryToast.relationshipUpdated(relationshipName);
          }
          resolve();
        },
        onError: (error: any) => {
  ...
  const handleBulkRelationshipUpdate = async (entityIds: number[], relationshipName: string, newValue: number | null) => {
    ...
    try {
      ...
      if (successCount > 0) {
        ...
        callCategoryToast.custom.success(
      ...
      if (errorCount === entityIds.length) {
        ...
        callCategoryToast.custom.warning(
          "⚠️ Partial Success",
        ...
        <div className="table-scroll overflow-x-auto">
          <Table className="w-full min-w-[600px]">
            ...
            {isLoading ? (
              ...
              data.map((callCategory) => (
                <CallCategoryTableRow
                  key={callCategory.id}
                  callCategory={callCategory}
                  onDelete={handleDelete}
                  isDeleting={isDeleting}
                  isSelected={selectedRows.has(callCategory.id || 0)}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              selected call categories and remove their data from the server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
