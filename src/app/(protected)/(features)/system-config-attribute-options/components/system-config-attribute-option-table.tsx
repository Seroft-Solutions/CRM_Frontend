...
export function SystemConfigAttributeOptionTable() {
  ...
  const exportToCSV = () => {
        ...
        visibleColumns
          .map((col) => {
            ...
            if (col.type === 'field') {
              ...
              if (col.id === 'attribute' && relationship) {
                value = relationship.name || relationship.label || relationship.id || '';
              }
            }
    ...
    link.href = url;
    link.download = `system-config-attribute-options-export-${new Date().toISOString().split('T')[0]}.csv`;
