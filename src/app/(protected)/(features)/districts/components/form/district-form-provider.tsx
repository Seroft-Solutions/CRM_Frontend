...
interface DistrictFormProviderProps {
...
export function DistrictFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: DistrictFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = districtFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a DistrictFormProvider');
  }
  return context;
}
