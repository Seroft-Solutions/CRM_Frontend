...
import {
  AdvancedPagination,
  usePaginationState,
} from '@/app/(protected)/(features)/districts/components/table/advanced-pagination';
...
export function DistrictTable() {
  ...
  const { mutate: deleteEntity, isPending: isDeleting } = useDeleteDistrict({
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.filter((district) => district.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(
        ...
        (old: any[]) => old?.filter((district) => !selectedRows.has(district.id || 0))
      );
      ...
      districtToast.bulkDeleted(selectedRows.size);
      setSelectedRows(new Set());
        ...
        <AlertDialog open={showBulkDeleteDialog} onOpenChange={setShowBulkDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the selected districts
        ...
        <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the district and remove
