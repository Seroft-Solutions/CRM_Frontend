...
import {
  ...
  useGetAllRoles,
  useSearchRoles,
  useUpdateRole} from '@/core/api/generated/spring/endpoints/role-resource/role-resource.gen';
...
const COLUMN_VISIBILITY_KEY = 'role-table-columns';
...
export function RoleTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'role-table-columns';
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `role-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': RoleDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': RoleDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': RoleDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': RoleDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': RoleDTOStatus.ACTIVE };
  ...
  const buildFilterParams = () => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value !== undefined && value !== '' && value !== null) {
        ...
        } else if (key === 'keycloakRoleId') {
          if (typeof value === 'string' && value.trim() !== '') {
            params['keycloakRoleId.contains'] = value;
          }
        } else if (key === 'name') {
          if (typeof value === 'string' && value.trim() !== '') {
  ...
  const { data: countData } = useCountRoles(filterParams, {
    query: {
      enabled: true,
      staleTime: 0,
      refetchOnWindowFocus: true}});
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(
            ...
            (old: any[]) =>
              old.map((role) => (role.id === variables.id ? { ...role, ...variables.data } : role))
          );
        }
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.map((role) => (role.id === variables.id ? data : role))
        );
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateRole({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllRoles'] });
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => {
            ...
            if (currentStatusFilter === newStatus || activeStatusTab === 'all') {
              ...
              return old.map((role) =>
                role.id === variables.id ? { ...role, ...variables.data } : role
              );
            } else {
      ...
      onSuccess: (data, variables) => {
        ...
        roleToast.custom.success(`Status Updated`, `Role status changed to ${statusLabel}`);
        ...
        if (currentStatusFilter !== newStatus && activeStatusTab !== 'all') {
          ...
          queryClient.setQueryData(['countRoles', filterParams], (old: number) =>
            Math.max(0, (old || 0) - 1)
          );
        }
      },
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: RoleDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue = RoleDTOStatus[bulkNewStatus as keyof typeof RoleDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
  ...
  const handleRelationshipUpdate = async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (!currentEntity) {
        ...
        reject(new Error('Role not found in current data'));
        return;
      }
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) => old?.map((role) => (role.id === entityId ? serverResponse : role))
              );
            ...
            if (!isBulkOperation) {
              roleToast.relationshipUpdated(relationshipName);
            }
            resolve();
          },
          onError: (error: any) => {
                ...
                {isLoading ? (
                  ...
                  data.map((role) => (
                    <RoleTableRow
                      ...
                      isSelected={selectedRows.has(role.id || 0)}
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected roles to "Archived". They will no longer
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected roles to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status to "Archived". The role will no longer appear in the
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this role to "
