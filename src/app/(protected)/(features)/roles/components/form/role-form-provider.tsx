...
export function RoleFormProvider({ children, id, onSuccess, onError }: RoleFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = roleFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a RoleFormProvider');
  }
  return context;
}
