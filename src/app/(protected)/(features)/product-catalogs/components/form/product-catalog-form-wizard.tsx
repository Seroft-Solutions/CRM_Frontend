...
import { ProductCatalogFormProvider, useEntityForm } from './product-catalog-form-provider';
...
function ProductCatalogFormContent({ id }: ProductCatalogFormProps) {
  const router = useRouter();
  const isNew = !id;
  const { state, actions, form, navigation, config } = useEntityForm();
  const { navigateBackToReferrer, hasReferrer } = useCrossFormNavigation();

  const { data: entity, isLoading: isLoadingEntity } = useGetProductCatalog(id || 0, {
    query: {
      enabled: !!id,
      queryKey: ['get-product-catalog', id],
    },
  });
    ...
    return (
      <div className="text-center p-8">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          1. Run: <code>node src/core/step-generator.js ProductCatalog</code>
          <br />
  ...
  const handleCancel = () => {
    if (hasReferrer()) {
      ...
      const backRoute = returnUrl || '/product-catalogs';
...
export function ProductCatalogForm({ id }: ProductCatalogFormProps) {
  const router = useRouter();
  ...
  const { mutate: createEntity, isPending: isCreating } = useCreateProductCatalog({
    mutation: {
      onSuccess: (data) => {
        ...
        if (hasReferrer() && entityId) {
          ...
          navigateBackToReferrer(entityId, 'ProductCatalog');
        } else {
          setIsRedirecting(true);
  ...
  const { mutate: updateEntity, isPending: isUpdating } = useUpdateProductCatalog({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ['getAllProductCatalogs'],
  ...
  return (
    <ProductCatalogFormProvider
      id={id}
      onSuccess={async (transformedData) => {
        const { ...productCatalogData } = transformedData as any;

        if (isNew) {
          createEntity({ data: productCatalogData as any });
        } else if (id) {
          const entityData = { ...productCatalogData, id };
    ...
    >
      <ProductCatalogFormContent id={id} />
    </ProductCatalogFormProvider>
  );
}
