...
import { productCatalogFormConfig } from './product-catalog-form-config';
...
interface ProductCatalogFormProviderProps {
...
export function ProductCatalogFormProvider({
  children,
  id,
  onSuccess,
  onError,
}: ProductCatalogFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = productCatalogFormConfig;
      ...
      if (savedStateStr) {
        try {
          ...
          if (isRecent && isSameSession && isSameEntity && isCrossFormState) {
            ...
            if (!suppressToast) {
              productCatalogToast.formRestored();
            }
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a ProductCatalogFormProvider');
  }
  return context;
}
