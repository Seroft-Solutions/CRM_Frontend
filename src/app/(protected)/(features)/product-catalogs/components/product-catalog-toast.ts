...
export const productCatalogToast = {
  created: (entityName?: string) =>
    toast.success('✅ Success!', {
      description: `${entityName || 'Product Catalog'} created successfully`,
      action: {
        label: 'View',
        onClick: () => (window.location.href = '/product-catalogs'),
      },
    }),

  updated: (entityName?: string) =>
    toast.success('✅ Updated!', {
      description: `${entityName || 'Product Catalog'} updated successfully`,
      action: {
        label: 'View All',
        onClick: () => (window.location.href = '/product-catalogs'),
      },
  ...
  deleted: (entityName?: string) =>
    toast.success('🗑️ Deleted!', {
      description: `${entityName || 'Product Catalog'} deleted successfully`,
    }),

  bulkDeleted: (count: number) =>
    toast.success('🗑️ Bulk Delete!', {
      description: `${count} product catalogs deleted successfully`,
    }),
  ...
  exported: (format: string) =>
    toast.success('📤 Exported!', {
      description: `Product Catalogs exported to ${format} successfully`,
    }),

  imported: (count: number) =>
    toast.success('📥 Imported!', {
      description: `${count} product catalogs imported successfully`,
    }),

  createError: (error?: string) =>
    toast.error('❌ Creation Failed', {
      description: error || `Failed to create product catalog. Please try again.`,
      action: {
        label: 'Retry',
  ...
  updateError: (error?: string) =>
    toast.error('❌ Update Failed', {
      description: error || `Failed to update product catalog. Please try again.`,
      action: {
        label: 'Retry',
        onClick: () => window.location.reload(),
      },
  ...
  deleteError: (error?: string) =>
    toast.error('❌ Delete Failed', {
      description: error || `Failed to delete product catalog. Please try again.`,
    }),

  bulkDeleteError: (error?: string) =>
    toast.error('❌ Bulk Delete Failed', {
      description: error || `Failed to delete selected product catalogs. Please try again.`,
    }),
...
export const handleProductCatalogError = (error: any) => {
  const errorMessage =
    error?.response?.data?.message || error?.message || 'An unexpected error occurred';

  if (error?.response?.status === 403) {
    productCatalogToast.permissionError();
  } else if (error?.response?.status === 422) {
    const validationErrors = error?.response?.data?.fieldErrors;
    const fields = validationErrors ? Object.keys(validationErrors) : [];
    productCatalogToast.validationError(fields);
  } else if (error?.code === 'NETWORK_ERROR') {
    productCatalogToast.networkError();
  } else {
    toast.error('❌ Error', {
