...
export function MeetingReminderDetails({ id }: MeetingReminderDetailsProps) {
  const router = useRouter();
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  const formConfig = meetingReminderFormConfig;

  const { data: entity, isLoading } = useGetMeetingReminder(id, {
    query: {
      enabled: !!id,
    },
  });

  const { mutate: deleteEntity } = useDeleteMeetingReminder({
    mutation: {
      onSuccess: () => {
