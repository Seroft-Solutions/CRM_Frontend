...
import { handleMeetingReminderError, meetingReminderToast } from './meeting-reminder-toast';
...
import {
  ...
  useUpdateMeetingReminder} from '@/core/api/generated/spring/endpoints/meeting-reminder-resource/meeting-reminder-resource.gen';
...
export function MeetingReminderTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'meeting-reminder-table-columns';
  ...
  const statusOptions = [
    {
      value: MeetingReminderDTOStatus.DRAFT,
      label: transformEnumValue('DRAFT'),
      color: 'bg-gray-100 text-gray-800'},
    {
      value: MeetingReminderDTOStatus.ACTIVE,
      label: transformEnumValue('ACTIVE'),
      color: 'bg-green-100 text-green-800'},
    {
      value: MeetingReminderDTOStatus.INACTIVE,
      label: transformEnumValue('INACTIVE'),
      color: 'bg-yellow-100 text-yellow-800'},
    {
      value: MeetingReminderDTOStatus.ARCHIVED,
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': MeetingReminderDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': MeetingReminderDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': MeetingReminderDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': MeetingReminderDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': MeetingReminderDTOStatus.ACTIVE };
    ...
    mutation: {
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) =>
            old?.map((meetingReminder) =>
              meetingReminder.id === variables.id ? data : meetingReminder
            )
        );
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateMeetingReminder({
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => {
            ...
            if (currentStatusFilter === newStatus || activeStatusTab === 'all') {
              ...
              return old.map((meetingReminder) =>
                meetingReminder.id === variables.id
                  ? { ...meetingReminder, ...variables.data }
                  : meetingReminder
              );
              ...
              return old.filter((meetingReminder) => meetingReminder.id !== variables.id);
            }
          }
        );
      ...
      onSuccess: (data, variables) => {
        ...
        meetingReminderToast.custom.success(
          `Status Updated`,
          `MeetingReminder status changed to ${statusLabel}`
        );
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: MeetingReminderDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue =
        MeetingReminderDTOStatus[bulkNewStatus as keyof typeof MeetingReminderDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) =>
                  old?.map((meetingReminder) =>
                    meetingReminder.id === entityId ? serverResponse : meetingReminder
                  )
              );
    ...
    <>
      ...
      <div className="w-full space-y-4">
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected meeting reminders to "Archived". They
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected meeting reminders to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Archive this meetingreminder?</AlertDialogTitle>
              <AlertDialogDescription>
                This will change the status to "Archived". The meetingreminder will no longer appear
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this meetingreminder to "
