...
import {
  AdvancedPagination,
  usePaginationState,
} from '@/app/(protected)/(features)/meeting-reminders/components/table/advanced-pagination';
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.filter((meetingReminder) => meetingReminder.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(
        ...
        (old: any[]) => old?.filter((meetingReminder) => !selectedRows.has(meetingReminder.id || 0))
      );
