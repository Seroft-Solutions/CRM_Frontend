...
// Toast notification utilities for MeetingReminder
export const meetingReminderToast = {
  // Success messages
  created: (entityName?: string) =>
    toast.success('✅ Success!', {
      description: `${entityName || 'Meeting Reminder'} created successfully`,
      action: {
        label: 'View',
        onClick: () => (window.location.href = '/meeting-reminders'),
      },
    }),

  updated: (entityName?: string) =>
    toast.success('✅ Updated!', {
      description: `${entityName || 'Meeting Reminder'} updated successfully`,
      action: {
        label: 'View All',
        onClick: () => (window.location.href = '/meeting-reminders'),
      },
  ...
  deleted: (entityName?: string) =>
    toast.success('🗑️ Deleted!', {
      description: `${entityName || 'Meeting Reminder'} deleted successfully`,
    }),

  bulkDeleted: (count: number) =>
    toast.success('🗑️ Bulk Delete!', {
      description: `${count} meeting reminders deleted successfully`,
    }),
  ...
  exported: (format: string) =>
    toast.success('📤 Exported!', {
      description: `Meeting Reminders exported to ${format} successfully`,
    }),

  imported: (count: number) =>
    toast.success('📥 Imported!', {
      description: `${count} meeting reminders imported successfully`,
    }),

  // Error messages
  createError: (error?: string) =>
    toast.error('❌ Creation Failed', {
      description: error || `Failed to create meetingreminder. Please try again.`,
      action: {
        label: 'Retry',
  ...
  updateError: (error?: string) =>
    toast.error('❌ Update Failed', {
      description: error || `Failed to update meetingreminder. Please try again.`,
      action: {
        label: 'Retry',
        onClick: () => window.location.reload(),
      },
  ...
  deleteError: (error?: string) =>
    toast.error('❌ Delete Failed', {
      description: error || `Failed to delete meetingreminder. Please try again.`,
    }),

  bulkDeleteError: (error?: string) =>
    toast.error('❌ Bulk Delete Failed', {
      description: error || `Failed to delete selected meeting reminders. Please try again.`,
    }),
...
export const handleMeetingReminderError = (error: any) => {
  const errorMessage =
    error?.response?.data?.message || error?.message || 'An unexpected error occurred';

  if (error?.response?.status === 403) {
    meetingReminderToast.permissionError();
  } else if (error?.response?.status === 422) {
    const validationErrors = error?.response?.data?.fieldErrors;
    const fields = validationErrors ? Object.keys(validationErrors) : [];
    meetingReminderToast.validationError(fields);
  } else if (error?.code === 'NETWORK_ERROR') {
    meetingReminderToast.networkError();
  } else {
    toast.error('❌ Error', {
