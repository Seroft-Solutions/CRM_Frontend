...
interface StateFormProviderProps {
...
export function StateFormProvider({ children, id, onSuccess, onError }: StateFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = stateFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a StateFormProvider');
  }
  return context;
}
