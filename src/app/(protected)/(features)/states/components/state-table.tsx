...
import { StateSearchAndFilters } from "@/app/(protected)/(features)/states/components/table/state-search-filters";
import { StateTableHeader } from "@/app/(protected)/(features)/states/components/table/state-table-header";
import { StateTableRow } from "@/app/(protected)/(features)/states/components/table/state-table-row";
import { BulkRelationshipAssignment } from "@/app/(protected)/(features)/states/components/table/bulk-relationship-assignment";
import { AdvancedPagination, usePaginationState } from "@/app/(protected)/(features)/states/components/table/advanced-pagination";
...
const COLUMN_VISIBILITY_KEY = 'state-table-columns'; // v2 to force reset for auditing fields
...
export function StateTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'state-table-columns'; // Old key without version
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `state-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const buildFilterParams = () => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (value !== undefined && value !== "" && value !== null) {
        ...
        // Handle country text filter with contains
        else if (key === 'country') {
          if (typeof value === 'string' && value.trim() !== '') {
            params['country.contains'] = value;
          }
        }
  ...
  const { mutate: updateEntity, isPending: isUpdating } = useUpdateState({
    mutation: {
      onMutate: async (variables) => {
        ...
        const previousData = queryClient.getQueryData(['getAllStates', {
          page: apiPage,
          size: pageSize,
          sort: [`${sort},${order}`],
          ...filterParams,
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(['getAllStates', {
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
            ...filterParams,
        ...
        if (searchTerm) {
          queryClient.setQueryData(['searchStates', {
            query: searchTerm,
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
  ...
  const { mutate: deleteEntity, isPending: isDeleting } = useDeleteState({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllStates'] });
        ...
        queryClient.setQueryData(['getAllStates', {
          ...
          old?.filter(state => state.id !== variables.id)
        );
      ...
      onSuccess: () => {
        stateToast.deleted();
        // Update count cache
        queryClient.setQueryData(['countStates', filterParams], (old: number) => 
          Math.max(0, (old || 0) - 1)
        );
      },
      onError: (error, variables, context) => {
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(['getAllStates', {
        ...
        old?.filter(state => !selectedRows.has(state.id || 0))
      );
  ...
  const handleRelationshipUpdate = async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (!currentEntity) {
        ...
        reject(new Error('State not found in current data'));
        return;
      }
            ...
            {isLoading ? (
              ...
              data.map((state) => (
                <StateTableRow
                  ...
                  isSelected={selectedRows.has(state.id || 0)}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              selected states and remove their data from the server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
