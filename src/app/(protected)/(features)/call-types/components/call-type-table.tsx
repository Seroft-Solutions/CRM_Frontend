...
import {
  callTypeToast,
  handleCallTypeError,
} from '@/app/(protected)/(features)/call-types/components/call-type-toast';
...
import { CallTypeSearchAndFilters } from '@/app/(protected)/(features)/call-types/components/table/call-type-search-filters';
import { CallTypeTableHeader } from '@/app/(protected)/(features)/call-types/components/table/call-type-table-header';
import { CallTypeTableRow } from '@/app/(protected)/(features)/call-types/components/table/call-type-table-row';
import { BulkRelationshipAssignment } from '@/app/(protected)/(features)/call-types/components/table/bulk-relationship-assignment';
import {
  AdvancedPagination,
  usePaginationState,
} from '@/app/(protected)/(features)/call-types/components/table/advanced-pagination';
...
export function CallTypeTable() {
  ...
  const { mutate: deleteEntity, isPending: isDeleting } = useDeleteCallType({
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.filter((callType) => callType.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(
        ...
        (old: any[]) => old?.filter((callType) => !selectedRows.has(callType.id || 0))
      );
      ...
      callTypeToast.bulkDeleted(selectedRows.size);
      setSelectedRows(new Set());
        ...
        <AlertDialog open={showBulkDeleteDialog} onOpenChange={setShowBulkDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the selected call types
        ...
        <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the calltype and remove
