...
import { AdvancedPagination, usePaginationState } from "@/app/(protected)/(features)/call-types/components/table/advanced-pagination";
...
export function CallTypeTable() {
  ...
  const { data: countData } = useCountCallTypes(
    filterParams,
    {
      query: {
        enabled: true,
  ...
  const { mutate: updateEntity, isPending: isUpdating } = useUpdateCallType({
    mutation: {
      onMutate: async (variables) => {
        ...
        const previousData = queryClient.getQueryData(['getAllCallTypes', {
          page: apiPage,
          size: pageSize,
          sort: [`${sort},${order}`],
          ...filterParams,
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(['getAllCallTypes', {
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
            ...filterParams,
        ...
        if (searchTerm) {
          queryClient.setQueryData(['searchCallTypes', {
            query: searchTerm,
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(['getAllCallTypes', {
          ...
          old?.filter(callType => callType.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(['getAllCallTypes', {
        ...
        old?.filter(callType => !selectedRows.has(callType.id || 0))
      );
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              selected call types and remove their data from the server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
