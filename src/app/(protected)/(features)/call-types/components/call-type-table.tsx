...
import {
  ...
  useGetAllCallTypes,
  useSearchCallTypes,
  useUpdateCallType} from '@/core/api/generated/spring/endpoints/call-type-resource/call-type-resource.gen';
...
const COLUMN_VISIBILITY_KEY = 'call-type-table-columns';
...
export function CallTypeTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'call-type-table-columns';
  ...
  const handleRefresh = async () => {
    try {
      await queryClient.invalidateQueries({
        queryKey: ['getAllCallTypes'],
        refetchType: 'active'});
      await queryClient.invalidateQueries({
        queryKey: ['countCallTypes'],
        refetchType: 'active'});

      await queryClient.invalidateQueries({
        queryKey: ['searchCallTypes'],
        refetchType: 'active'});
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `call-type-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const statusOptions = [
    {
      value: CallTypeDTOStatus.DRAFT,
      label: transformEnumValue('DRAFT'),
      color: 'bg-gray-100 text-gray-800'},
    {
      value: CallTypeDTOStatus.ACTIVE,
      label: transformEnumValue('ACTIVE'),
      color: 'bg-green-100 text-green-800'},
    {
      value: CallTypeDTOStatus.INACTIVE,
      label: transformEnumValue('INACTIVE'),
      color: 'bg-yellow-100 text-yellow-800'},
    {
      value: CallTypeDTOStatus.ARCHIVED,
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': CallTypeDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': CallTypeDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': CallTypeDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': CallTypeDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': CallTypeDTOStatus.ACTIVE };
  ...
  const { data: countData } = useCountCallTypes(filterParams, {
    query: {
      enabled: true,
      staleTime: 0,
      refetchOnWindowFocus: true}});
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(
            ...
            (old: any[]) =>
              old.map((callType) =>
                callType.id === variables.id ? { ...callType, ...variables.data } : callType
              )
          );
        }
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.map((callType) => (callType.id === variables.id ? data : callType))
        );
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateCallType({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllCallTypes'] });
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => {
            ...
            if (currentStatusFilter === newStatus || activeStatusTab === 'all') {
              ...
              return old.map((callType) =>
                callType.id === variables.id ? { ...callType, ...variables.data } : callType
              );
            } else {
              console.log(`Removing item ${variables.id} from current view`);
              return old.filter((callType) => callType.id !== variables.id);
            }
          }
        );
      ...
      onSuccess: (data, variables) => {
        ...
        callTypeToast.custom.success(`Status Updated`, `CallType status changed to ${statusLabel}`);
        ...
        if (currentStatusFilter !== newStatus && activeStatusTab !== 'all') {
          ...
          queryClient.setQueryData(['countCallTypes', filterParams], (old: number) =>
            Math.max(0, (old || 0) - 1)
          );
        }
      },
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: CallTypeDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue = CallTypeDTOStatus[bulkNewStatus as keyof typeof CallTypeDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
  ...
  const handleRelationshipUpdate = async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (!currentEntity) {
        ...
        reject(new Error('CallType not found in current data'));
        return;
      }
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) =>
                  old?.map((callType) => (callType.id === entityId ? serverResponse : callType))
              );
            ...
            if (!isBulkOperation) {
              callTypeToast.relationshipUpdated(relationshipName);
            }
            resolve();
          },
          onError: (error: any) => {
    ...
    <>
      ...
      <div className="w-full space-y-4">
                ...
                {isLoading ? (
                  ...
                  data.map((callType) => (
                    <CallTypeTableRow
                      ...
                      isSelected={selectedRows.has(callType.id || 0)}
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected call types to "Archived". They will no
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected call types to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Archive this calltype?</AlertDialogTitle>
              <AlertDialogDescription>
                This will change the status to "Archived". The calltype will no longer appear in the
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this calltype to "
