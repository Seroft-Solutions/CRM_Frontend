...
} from '@/app/(protected)/(features)/call-statuses/components/call-status-toast';
...
import { CallStatusSearchAndFilters } from '@/app/(protected)/(features)/call-statuses/components/table/call-status-search-filters';
import { CallStatusTableHeader } from '@/app/(protected)/(features)/call-statuses/components/table/call-status-table-header';
import { CallStatusTableRow } from '@/app/(protected)/(features)/call-statuses/components/table/call-status-table-row';
import { BulkRelationshipAssignment } from '@/app/(protected)/(features)/call-statuses/components/table/bulk-relationship-assignment';
import {
  AdvancedPagination,
  usePaginationState,
} from '@/app/(protected)/(features)/call-statuses/components/table/advanced-pagination';
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.filter((callStatus) => callStatus.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(
        ...
        (old: any[]) => old?.filter((callStatus) => !selectedRows.has(callStatus.id || 0))
      );
        ...
        <AlertDialog open={showBulkDeleteDialog} onOpenChange={setShowBulkDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the selected call
                statuses and remove their data from the server.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the callstatus and remove
