...
import {
  ...
  useUpdateCallStatus} from '@/core/api/generated/spring/endpoints/call-status-resource/call-status-resource.gen';
...
export function CallStatusTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'call-status-table-columns';
  ...
  const handleRefresh = async () => {
    try {
      await queryClient.invalidateQueries({
        queryKey: ['getAllCallStatuses'],
        refetchType: 'active'});
      await queryClient.invalidateQueries({
        queryKey: ['countCallStatuses'],
        refetchType: 'active'});

      await queryClient.invalidateQueries({
        queryKey: ['searchCallStatuses'],
        refetchType: 'active'});
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `call-status-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const statusOptions = [
    {
      value: CallStatusDTOStatus.DRAFT,
      label: transformEnumValue('DRAFT'),
      color: 'bg-gray-100 text-gray-800'},
    {
      value: CallStatusDTOStatus.ACTIVE,
      label: transformEnumValue('ACTIVE'),
      color: 'bg-green-100 text-green-800'},
    {
      value: CallStatusDTOStatus.INACTIVE,
      label: transformEnumValue('INACTIVE'),
      color: 'bg-yellow-100 text-yellow-800'},
    {
      value: CallStatusDTOStatus.ARCHIVED,
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': CallStatusDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': CallStatusDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': CallStatusDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': CallStatusDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': CallStatusDTOStatus.ACTIVE };
  ...
  const { data: countData } = useCountCallStatuses(filterParams, {
    query: {
      enabled: true,
      staleTime: 0,
      refetchOnWindowFocus: true}});
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(
            ...
            (old: any[]) =>
              old.map((callStatus) =>
                callStatus.id === variables.id ? { ...callStatus, ...variables.data } : callStatus
              )
          );
        }
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) =>
            old?.map((callStatus) => (callStatus.id === variables.id ? data : callStatus))
        );
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateCallStatus({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllCallStatuses'] });
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => {
            ...
            if (currentStatusFilter === newStatus || activeStatusTab === 'all') {
              ...
              return old.map((callStatus) =>
                callStatus.id === variables.id ? { ...callStatus, ...variables.data } : callStatus
              );
            } else {
              console.log(`Removing item ${variables.id} from current view`);
              return old.filter((callStatus) => callStatus.id !== variables.id);
            }
          }
        );
      ...
      onSuccess: (data, variables) => {
        ...
        callStatusToast.custom.success(
          `Status Updated`,
          `CallStatus status changed to ${statusLabel}`
        );
        ...
        if (currentStatusFilter !== newStatus && activeStatusTab !== 'all') {
          ...
          queryClient.setQueryData(['countCallStatuses', filterParams], (old: number) =>
            Math.max(0, (old || 0) - 1)
          );
        }
      },
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: CallStatusDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue = CallStatusDTOStatus[bulkNewStatus as keyof typeof CallStatusDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) =>
                  old?.map((callStatus) =>
                    callStatus.id === entityId ? serverResponse : callStatus
                  )
              );
    ...
    <>
      ...
      <div className="w-full space-y-4">
                ...
                {isLoading ? (
                  ...
                  data.map((callStatus) => (
                    <CallStatusTableRow
                      ...
                      isSelected={selectedRows.has(callStatus.id || 0)}
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected call statuses to "Archived". They will
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected call statuses to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Archive this callstatus?</AlertDialogTitle>
              <AlertDialogDescription>
                This will change the status to "Archived". The callstatus will no longer appear in
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this callstatus to "
