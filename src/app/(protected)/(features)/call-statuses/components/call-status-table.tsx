...
import { callStatusToast, handleCallStatusError } from "@/app/(protected)/(features)/call-statuses/components/call-status-toast";
...
import { AdvancedPagination, usePaginationState } from "@/app/(protected)/(features)/call-statuses/components/table/advanced-pagination";
  ...
  const { mutate: updateEntity, isPending: isUpdating } = useUpdateCallStatus({
    mutation: {
      onMutate: async (variables) => {
        ...
        const previousData = queryClient.getQueryData(['getAllCallStatuses', {
          page: apiPage,
          size: pageSize,
          sort: [`${sort},${order}`],
          ...filterParams,
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(['getAllCallStatuses', {
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
            ...filterParams,
        ...
        if (searchTerm) {
          queryClient.setQueryData(['searchCallStatuses', {
            query: searchTerm,
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(['getAllCallStatuses', {
          ...
          old?.filter(callStatus => callStatus.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(['getAllCallStatuses', {
        ...
        old?.filter(callStatus => !selectedRows.has(callStatus.id || 0))
      );
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              selected call statuses and remove their data from the server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
