...
import { callStatusFormConfig } from "@/app/(protected)/(features)/call-statuses/components/form/call-status-form-config";
import { callStatusFormSchema } from "@/app/(protected)/(features)/call-statuses/components/form/call-status-form-schema";
...
interface CallStatusFormProviderProps {
...
export function CallStatusFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: CallStatusFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = callStatusFormConfig;
  ...
  const form = useForm<Record<string, any>>({
    resolver: zodResolver(callStatusFormSchema),
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a CallStatusFormProvider');
  }
  return context;
}
