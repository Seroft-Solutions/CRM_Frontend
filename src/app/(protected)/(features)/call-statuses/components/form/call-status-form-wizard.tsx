...
import { CallStatusFormProvider, useEntityForm } from "@/app/(protected)/(features)/call-statuses/components/form/call-status-form-provider";
import { FormProgressIndicator } from "@/app/(protected)/(features)/call-statuses/components/form/form-progress-indicator";
import { FormStepRenderer } from "@/app/(protected)/(features)/call-statuses/components/form/form-step-renderer";
import { FormNavigation } from "@/app/(protected)/(features)/call-statuses/components/form/form-navigation";
import { FormStateManager } from "@/app/(protected)/(features)/call-statuses/components/form/form-state-manager";
import { FormErrorsDisplay } from "@/components/form-errors-display";
import { Form } from "@/components/ui/form";
import { Card, CardContent } from "@/components/ui/card";
// Import generated step components (uncommented by step generator)
// import { stepComponents } from "@/app/(protected)/(features)/call-statuses/components/form/steps";
import { 
  useCreateCallStatus,
  useUpdateCallStatus,
  useGetCallStatus,
...
function CallStatusFormContent({ id }: CallStatusFormProps) {
    ...
    return (
      <div className="text-center p-8">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          1. Run: <code>node src/core/step-generator.js CallStatus</code><br/>
  ...
  const handleCancel = () => {
    if (hasReferrer()) {
      ...
      const backRoute = returnUrl || "/call-statuses";
    ...
    mutation: {
      onSuccess: (data) => {
        ...
        if (hasReferrer() && entityId) {
          ...
          navigateBackToReferrer(entityId, 'CallStatus');
        } else {
          setIsRedirecting(true);
