...
import {
  ...
  useUpdateOrganization} from '@/core/api/generated/spring/endpoints/organization-resource/organization-resource.gen';
...
const COLUMN_VISIBILITY_KEY = 'organization-table-columns';
...
export function OrganizationTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'organization-table-columns';
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `organization-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const statusOptions = [
    {
      value: OrganizationDTOStatus.DRAFT,
      label: transformEnumValue('DRAFT'),
      color: 'bg-gray-100 text-gray-800'},
    {
      value: OrganizationDTOStatus.ACTIVE,
      label: transformEnumValue('ACTIVE'),
      color: 'bg-green-100 text-green-800'},
    {
      value: OrganizationDTOStatus.INACTIVE,
      label: transformEnumValue('INACTIVE'),
      color: 'bg-yellow-100 text-yellow-800'},
    {
      value: OrganizationDTOStatus.ARCHIVED,
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': OrganizationDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': OrganizationDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': OrganizationDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': OrganizationDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': OrganizationDTOStatus.ACTIVE };
    ...
    mutation: {
          ...
          queryClient.setQueryData(
            ...
            (old: any[]) =>
              old.map((organization) =>
                organization.id === variables.id
                  ? { ...organization, ...variables.data }
                  : organization
              )
          );
        }
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) =>
            old?.map((organization) => (organization.id === variables.id ? data : organization))
        );
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateOrganization({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllOrganizations'] });
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => {
            ...
            if (currentStatusFilter === newStatus || activeStatusTab === 'all') {
              ...
              return old.filter((organization) => organization.id !== variables.id);
            }
          }
        );
      ...
      onSuccess: (data, variables) => {
        ...
        organizationToast.custom.success(
          `Status Updated`,
          `Organization status changed to ${statusLabel}`
        );
        ...
        if (currentStatusFilter !== newStatus && activeStatusTab !== 'all') {
          ...
          queryClient.setQueryData(['countOrganizations', filterParams], (old: number) =>
            Math.max(0, (old || 0) - 1)
          );
        }
      },
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: OrganizationDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue =
        OrganizationDTOStatus[bulkNewStatus as keyof typeof OrganizationDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) =>
                  old?.map((organization) =>
                    organization.id === entityId ? serverResponse : organization
                  )
              );
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected organizations to "Archived". They will
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected organizations to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status to "Archived". The organization will no longer appear in
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this organization to "
