...
import { AdvancedPagination, usePaginationState } from "@/app/(protected)/(features)/organizations/components/table/advanced-pagination";
  ...
  const { mutate: updateEntity, isPending: isUpdating } = useUpdateOrganization({
    mutation: {
      onMutate: async (variables) => {
        ...
        const previousData = queryClient.getQueryData(['getAllOrganizations', {
          page: apiPage,
          size: pageSize,
          sort: [`${sort},${order}`],
          ...filterParams,
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(['getAllOrganizations', {
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
            ...filterParams,
        ...
        if (searchTerm) {
          queryClient.setQueryData(['searchOrganizations', {
            query: searchTerm,
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(['getAllOrganizations', {
          ...
          old?.filter(organization => organization.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(['getAllOrganizations', {
        ...
        old?.filter(organization => !selectedRows.has(organization.id || 0))
      );
