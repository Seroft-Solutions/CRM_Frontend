...
interface OrganizationFormProviderProps {
...
export function OrganizationFormProvider({
  ...
  const isNew = !id;
  const config = organizationFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a OrganizationFormProvider');
  }
  return context;
}
