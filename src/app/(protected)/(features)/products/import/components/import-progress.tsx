...
export function ImportProgress({ jobId, onComplete }: ImportProgressProps) {
  ...
  const { data: progress, isLoading, error } = useGetImportProgress(jobId, {
    query: {
      refetchInterval: (query) => {
        const data = query.state.data;
        // Stop polling if completed or failed
        if (data?.status === ProductImportJobDTOStatus.COMPLETED || data?.status === ProductImportJobDTOStatus.FAILED) {
          return false;
        }
