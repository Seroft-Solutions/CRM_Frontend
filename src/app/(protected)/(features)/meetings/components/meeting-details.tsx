...
function RelationshipDisplayValue({ 
  ...
  // Get the appropriate hook for this relationship
    const { data: organizerData } = relConfig.name === 'organizer' ? 
    useGetAllUserProfiles({ page: 0, size: 1000 }, {
      query: {
        enabled: !!value && relConfig.name === 'organizer',
        staleTime: 5 * 60 * 1000,
      }
    }) : { data: null };
      const { data: assignedCustomerData } = relConfig.name === 'assignedCustomer' ? 
    useGetAllCustomers({ page: 0, size: 1000 }, {
      query: {
        enabled: !!value && relConfig.name === 'assignedCustomer',
        staleTime: 5 * 60 * 1000,
      }
    }) : { data: null };
      const { data: callData } = relConfig.name === 'call' ? 
    useGetAllCalls({ page: 0, size: 1000 }, {
      query: {
        enabled: !!value && relConfig.name === 'call',
        staleTime: 5 * 60 * 1000,
      }
    }) : { data: null };
  ...
  if (relConfig.name === 'assignedCustomer') {
    allData = assignedCustomerData;
  }
  if (relConfig.name === 'call') {
    allData = callData;
  }
