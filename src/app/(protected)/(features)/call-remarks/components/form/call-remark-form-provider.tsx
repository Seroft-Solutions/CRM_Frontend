...
interface CallRemarkFormProviderProps {
...
export function CallRemarkFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: CallRemarkFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = callRemarkFormConfig;
  ...
  const form = useForm<Record<string, any>>({
    resolver: zodResolver(callRemarkFormSchema),
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a CallRemarkFormProvider');
  }
  return context;
}
