...
import { subCallTypeFormConfig } from '@/app/(protected)/(features)/sub-call-types/components/form/sub-call-type-form-config';
import { subCallTypeFormSchema } from '@/app/(protected)/(features)/sub-call-types/components/form/sub-call-type-form-schema';
import {
  subCallTypeToast,
...
interface SubCallTypeFormProviderProps {
...
export function SubCallTypeFormProvider({
  children,
  id,
  onSuccess,
  onError,
}: SubCallTypeFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = subCallTypeFormConfig;
  ...
  const form = useForm<Record<string, any>>({
    resolver: zodResolver(subCallTypeFormSchema),
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a SubCallTypeFormProvider');
  }
  return context;
}
