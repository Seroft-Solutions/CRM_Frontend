...
// Toast notification utilities for ProductCategory
export const productCategoryToast = {
  // Success messages
  created: (entityName?: string) =>
    toast.success('✅ Success!', {
      description: `${entityName || 'Product Category'} created successfully`,
      action: {
        label: 'View',
  ...
  updated: (entityName?: string) =>
    toast.success('✅ Updated!', {
      description: `${entityName || 'Product Category'} updated successfully`,
      action: {
        label: 'View All',
        onClick: () => (window.location.href = '/product-categories'),
      },
  ...
  deleted: (entityName?: string) =>
    toast.success('🗑️ Deleted!', {
      description: `${entityName || 'Product Category'} deleted successfully`,
    }),

  bulkDeleted: (count: number) =>
    toast.success('🗑️ Bulk Delete!', {
      description: `${count} product categories deleted successfully`,
    }),
  ...
  exported: (format: string) =>
    toast.success('📤 Exported!', {
      description: `Product Categories exported to ${format} successfully`,
    }),

  imported: (count: number) =>
    toast.success('📥 Imported!', {
      description: `${count} product categories imported successfully`,
    }),

  // Error messages
  createError: (error?: string) =>
    toast.error('❌ Creation Failed', {
      description: error || `Failed to create productcategory. Please try again.`,
      action: {
        label: 'Retry',
  ...
  updateError: (error?: string) =>
    toast.error('❌ Update Failed', {
      description: error || `Failed to update productcategory. Please try again.`,
      action: {
        label: 'Retry',
        onClick: () => window.location.reload(),
      },
  ...
  deleteError: (error?: string) =>
    toast.error('❌ Delete Failed', {
      description: error || `Failed to delete productcategory. Please try again.`,
    }),

  bulkDeleteError: (error?: string) =>
    toast.error('❌ Bulk Delete Failed', {
      description: error || `Failed to delete selected product categories. Please try again.`,
    }),
...
export const handleProductCategoryError = (error: any) => {
  const errorMessage =
    error?.response?.data?.message || error?.message || 'An unexpected error occurred';

  if (error?.response?.status === 403) {
    productCategoryToast.permissionError();
  } else if (error?.response?.status === 422) {
    const validationErrors = error?.response?.data?.fieldErrors;
    const fields = validationErrors ? Object.keys(validationErrors) : [];
    productCategoryToast.validationError(fields);
  } else if (error?.code === 'NETWORK_ERROR') {
    productCategoryToast.networkError();
  } else {
    toast.error('❌ Error', {
