...
import { handleProductCategoryError, productCategoryToast } from './product-category-toast';
...
import {
  ...
  useUpdateProductCategory} from '@/core/api/generated/spring/endpoints/product-category-resource/product-category-resource.gen';
import { ProductCategoryTableHeader } from './table/product-category-table-header';
import { ProductCategoryTableRow } from './table/product-category-table-row';
...
const COLUMN_VISIBILITY_KEY = 'product-category-table-columns';
...
export function ProductCategoryTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'product-category-table-columns';
  ...
  const handleRefresh = async () => {
    try {
      await queryClient.invalidateQueries({
        ...
        refetchType: 'active'});
      await queryClient.invalidateQueries({
        queryKey: ['countProductCategories'],
        refetchType: 'active'});

      await queryClient.invalidateQueries({
        queryKey: ['searchProductCategories'],
        refetchType: 'active'});
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `product-category-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const statusOptions = [
    {
      value: ProductCategoryDTOStatus.DRAFT,
      label: transformEnumValue('DRAFT'),
      color: 'bg-gray-100 text-gray-800'},
    {
      value: ProductCategoryDTOStatus.ACTIVE,
      label: transformEnumValue('ACTIVE'),
      color: 'bg-green-100 text-green-800'},
    {
      value: ProductCategoryDTOStatus.INACTIVE,
      label: transformEnumValue('INACTIVE'),
      color: 'bg-yellow-100 text-yellow-800'},
    {
      value: ProductCategoryDTOStatus.ARCHIVED,
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': ProductCategoryDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': ProductCategoryDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': ProductCategoryDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': ProductCategoryDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': ProductCategoryDTOStatus.ACTIVE };
  ...
  const { data: countData } = useCountProductCategories(filterParams, {
    query: {
      enabled: true,
      staleTime: 0,
      refetchOnWindowFocus: true}});

  const { mutate: updateEntity, isPending: isUpdating } = useUpdateProductCategory({
    mutation: {
      onMutate: async (variables) => {
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(
            ...
            (old: any[]) =>
              old.map((productCategory) =>
                productCategory.id === variables.id
                  ? { ...productCategory, ...variables.data }
                  : productCategory
              )
          );
        }
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) =>
            old?.map((productCategory) =>
              productCategory.id === variables.id ? data : productCategory
            )
        );
      ...
      onError: (error, variables, context) => {
        ...
        handleProductCategoryError(error);
      },
      onSettled: async () => {
        await queryClient.invalidateQueries({
          queryKey: ['getAllProductCategories'],
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateProductCategory({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllProductCategories'] });
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => {
            ...
            if (currentStatusFilter === newStatus || activeStatusTab === 'all') {
              ...
              return old.map((productCategory) =>
                productCategory.id === variables.id
                  ? { ...productCategory, ...variables.data }
                  : productCategory
              );
              ...
              return old.filter((productCategory) => productCategory.id !== variables.id);
            }
          }
        );
      ...
      onSuccess: (data, variables) => {
        ...
        productCategoryToast.custom.success(
          `Status Updated`,
          `ProductCategory status changed to ${statusLabel}`
        );
        ...
        if (currentStatusFilter !== newStatus && activeStatusTab !== 'all') {
          ...
          queryClient.setQueryData(['countProductCategories', filterParams], (old: number) =>
            Math.max(0, (old || 0) - 1)
          );
        }
      },
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: ProductCategoryDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue =
        ProductCategoryDTOStatus[bulkNewStatus as keyof typeof ProductCategoryDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
  ...
  const handleRelationshipUpdate = async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (!currentEntity) {
        ...
        reject(new Error('ProductCategory not found in current data'));
        return;
      }
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) =>
                  old?.map((productCategory) =>
                    productCategory.id === entityId ? serverResponse : productCategory
                  )
              );
            ...
            if (!isBulkOperation) {
              productCategoryToast.relationshipUpdated(relationshipName);
            }
            resolve();
          },
          onError: (error: any) => {
    ...
    <>
      ...
      <div className="w-full space-y-4">
                ...
                {isLoading ? (
                  ...
                  data.map((productCategory) => (
                    <ProductCategoryTableRow
                      ...
                      isSelected={selectedRows.has(productCategory.id || 0)}
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected product categories to "Archived". They
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected product categories to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Archive this productcategory?</AlertDialogTitle>
              <AlertDialogDescription>
                This will change the status to "Archived". The productcategory will no longer appear
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this productcategory to "
