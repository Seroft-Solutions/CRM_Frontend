...
import { productCategoryFormConfig } from './form/product-category-form-config';
...
export function ProductCategoryDetails({ id }: ProductCategoryDetailsProps) {
  const router = useRouter();
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  // Get form config for step organization
  const formConfig = productCategoryFormConfig;

  // Fetch entity details
  const { data: entity, isLoading } = useGetProductCategory(id, {
    query: {
      enabled: !!id,
    },
  });
  ...
  // Delete mutation
  const { mutate: deleteEntity } = useDeleteProductCategory({
    mutation: {
      onSuccess: () => {
        productCategoryToast.deleted();
      ...
      <div className="mt-8 pt-6 border-t">
        <div className="flex flex-col sm:flex-row gap-3 sm:justify-end">
          <Button variant="outline" asChild className="flex items-center gap-2 justify-center">
            <Link href={`/product-categories/${id}/edit`}>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the productcategory and
