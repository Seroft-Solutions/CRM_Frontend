...
import { ProductCategoryFormProvider, useEntityForm } from './product-category-form-provider';
...
function ProductCategoryFormContent({ id }: ProductCategoryFormProps) {
  const router = useRouter();
  const isNew = !id;
  const { state, actions, form, navigation, config } = useEntityForm();
  const { navigateBackToReferrer, hasReferrer } = useCrossFormNavigation();

  const { data: entity, isLoading: isLoadingEntity } = useGetProductCategory(id || 0, {
    query: {
      enabled: !!id,
    ...
    return (
      <div className="text-center p-8">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          1. Run: <code>node src/core/step-generator.js ProductCategory</code>
          <br />
  ...
  const handleCancel = () => {
    if (hasReferrer()) {
      ...
      const backRoute = returnUrl || '/product-categories';
...
export function ProductCategoryForm({ id }: ProductCategoryFormProps) {
  const router = useRouter();
  ...
  const { mutate: createEntity, isPending: isCreating } = useCreateProductCategory({
    mutation: {
      onSuccess: (data) => {
        ...
        if (hasReferrer() && entityId) {
          ...
          navigateBackToReferrer(entityId, 'ProductCategory');
        } else {
          setIsRedirecting(true);
  ...
  return (
    <ProductCategoryFormProvider
      id={id}
      onSuccess={async (transformedData) => {
        const { ...productCategoryData } = transformedData as any;
        const productCategoryDataWithStatus = {
          ...productCategoryData,
          status: 'ACTIVE',
        };

        if (isNew) {
          createEntity({ data: productCategoryDataWithStatus as any });
        } else if (id) {
          const entityData = { ...productCategoryDataWithStatus, id };
    ...
    >
      <ProductCategoryFormContent id={id} />
    </ProductCategoryFormProvider>
  );
}
