...
import { productCategoryFormConfig } from './product-category-form-config';
import { productCategoryFormSchema } from './product-category-form-schema';
...
interface ProductCategoryFormProviderProps {
...
export function ProductCategoryFormProvider({
  children,
  id,
  onSuccess,
  onError,
}: ProductCategoryFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = productCategoryFormConfig;
      ...
      if (savedStateStr) {
        try {
          ...
          if (isRecent && isSameSession && isSameEntity && isCrossFormState) {
            ...
            if (!suppressToast) {
              productCategoryToast.formRestored();
            }
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a ProductCategoryFormProvider');
  }
  return context;
}
