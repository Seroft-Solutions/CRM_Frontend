...
export function OrderHistoryTable() {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-300 bg-white shadow-sm">
      <div className="flex items-center justify-between bg-slate-50 border-b border-slate-300 px-6 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-800">Complete Order History</h3>
                ...
                <TableRow key={entry.orderHistoryId} className="border-slate-200 hover:bg-sidebar-accent/5">
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="text-sm font-medium text-slate-800">#{entry.orderId}</div>
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className="ring-1 ring-blue-300 border-0 bg-blue-50/50 text-xs font-medium text-blue-700"
                    >
                      {entry.status}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    {entry.itemStatus ? (
                      <Badge variant="outline" className="ring-1 ring-cyan-300 border-0 bg-cyan-50/50 text-xs font-medium text-cyan-700">
                        {entry.itemStatus}
                      </Badge>
                    ) : (
                  ...
                  <TableCell>
                    {entry.notificationSent ? (
                      <Badge variant="outline" className="ring-1 ring-emerald-300 border-0 bg-emerald-50/50 text-xs font-medium text-emerald-700">
                  ...
                  <TableCell className="text-xs">
                    <div className="font-medium text-slate-700">{formatDateTime(entry.createdDate)}</div>
                    <div className="text-xs text-muted-foreground">By {entry.createdBy}</div>
                  </TableCell>
                  <TableCell className="text-xs">
                    <div className="font-medium text-slate-700">{formatDateTime(entry.lastUpdated || entry.createdDate)}</div>
      ...
      {!isLoading && !isError && totalCount > 0 && (
        <div className="flex flex-col items-center justify-between gap-4 border-t border-slate-300 bg-slate-50 px-6 py-3 sm:flex-row">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <label htmlFor="pageSize" className="text-xs font-medium text-slate-700">
                Rows per page:
              </label>
              <select
                id="pageSize"
                ...
                className="rounded-md border border-slate-300 bg-white px-2 py-1 text-xs font-medium text-slate-700 shadow-sm hover:border-slate-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
              >
