...
import type { OrderDetailItem, OrderRecord, OrderStatus } from '../data/order-data';
...
const statusTheme: Record<OrderStatus, { pill: string; dot: string }> = {
  ...
  Processing: { pill: 'bg-blue-500/10 text-blue-600 ring-blue-500/20', dot: 'bg-blue-500' },
  ...
  'Partially Approved': { pill: 'bg-orange-500/10 text-orange-600 ring-orange-500/20', dot: 'bg-orange-500' },
  Picked: { pill: 'bg-indigo-500/10 text-indigo-600 ring-indigo-500/20', dot: 'bg-indigo-500' },
  Packed: { pill: 'bg-violet-500/10 text-violet-600 ring-violet-500/20', dot: 'bg-violet-500' },
  Shipped: { pill: 'bg-cyan-500/10 text-cyan-600 ring-cyan-500/20', dot: 'bg-cyan-500' },
  Delivered: { pill: 'bg-emerald-500/10 text-emerald-600 ring-emerald-500/20', dot: 'bg-emerald-500' },
  Cancelled: { pill: 'bg-rose-500/10 text-rose-600 ring-rose-500/20', dot: 'bg-rose-500' },
...
export function OrderDetail({ order, headerSlot }: OrderDetailProps) {
  ...
  const deliveredQuantityByDetailId = useMemo(() => {
    const m = new Map<number, number>();
    generations.forEach((g) => {
      g.items?.forEach((i) => {
        if (typeof i.orderDetailId !== 'number') return;
        m.set(i.orderDetailId, (m.get(i.orderDetailId) ?? 0) + Math.max(0, i.deliveredQuantity ?? 0));
      });
    });
    return m;
  }, [generations]);
  ...
  const displayItems = useMemo(() =>
    order.items.map((item) => {
      const orderedQty = Math.max(0, item.quantity) + Math.max(0, item.backOrderQuantity) + (deliveredQuantityByDetailId.get(item.orderDetailId) ?? 0);
      const total = Math.max(orderedQty * item.itemPrice + item.itemTaxAmount, 0);
      return { ...item, orderedQuantity: orderedQty, displayTotal: total };
    }),
    [deliveredQuantityByDetailId, order.items]
  );

  const recalculatedBase = displayItems.reduce((s, i) => s + i.displayTotal, 0);
  const taxRate = order.orderTaxRate ?? 0;
  const discountAmount = (() => {
    if (!order.discountCode) return 0;
    const ship = order.shipping.shippingAmount ?? 0;
    const tot = order.orderTotalAmount ?? 0;
    const div = 1 + taxRate / 100;
    if (div <= 0) return 0;
    return Math.max(recalculatedBase - Math.max((tot - ship) / div, 0), 0);
  })();
  const taxableAmount = Math.max(recalculatedBase - discountAmount, 0);
  const taxAmount = (taxRate / 100) * taxableAmount;
  const total = Math.max(taxableAmount + (order.shipping.shippingAmount ?? 0) + taxAmount, 0);
    ...
    <div className="flex flex-col h-[calc(100vh-12px)] bg-slate-100 overflow-hidden border border-slate-300">
      ...
      <div className="grid grid-cols-5 divide-x divide-slate-200 border-b border-slate-300 bg-white">
        <Metric
          ...
          sub={`Base ${formatCurrency(recalculatedBase)}${discountAmount > 0 ? ` − Disc ${formatCurrency(discountAmount)}` : ''} + Tax ${formatCurrency(taxAmount)}`}
        ...
        <Metric icon={<User className="h-4 w-4 text-white" />} label="Customer" value={customerName} sub={customerPhone !== '—' ? customerPhone : customerEmail} accent="bg-violet-500" />
      ...
      <div className="flex-1 grid grid-cols-[1fr_280px] min-h-0">
        ...
        <div className="flex flex-col border-r border-slate-300 min-h-0">
          <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-300 bg-white">
            ...
            <div className="ml-auto flex items-center gap-1">
              <Button asChild size="sm" variant="ghost" className="h-6 px-2 text-[10px] gap-1 text-slate-500 hover:text-sidebar-accent-foreground">
                <Link href="/orders/back-to-manager"><Undo2 className="h-3 w-3" />Manager</Link>
              </Button>
              <Button asChild size="sm" variant="ghost" className="h-6 px-2 text-[10px] gap-1 text-slate-500 hover:text-sidebar-accent-foreground">
                <Link href={`/orders/${order.orderId}/fulfillment/history?from=order`}><History className="h-3 w-3" />History</Link>
              </Button>
              <Button asChild size="sm" className="h-6 px-2 text-[10px] gap-1 bg-sidebar-accent hover:bg-sidebar-accent/90 text-white">
                <Link href={`/orders/${order.orderId}/fulfillment?from=order`}><PackageCheck className="h-3 w-3" />Fulfill</Link>
              </Button>
            </div>
          </div>
                  ...
                  return (
                    <TableRow key={item.orderDetailId} className="border-b border-slate-200 hover:bg-sidebar-accent/5 transition-colors">
                      ...
                      <TableCell className="py-1.5 text-center">
                        <span className="inline-flex h-5 min-w-[22px] items-center justify-center rounded bg-slate-100 text-[12px] font-bold text-slate-700 px-1.5">{item.orderedQuantity}</span>
                      </TableCell>
          ...
          <div className="flex items-center justify-between px-4 py-2 border-t border-slate-300 bg-slate-900 text-white mt-auto">
            <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-300">
              {displayItems.length} item{displayItems.length !== 1 ? 's' : ''} · Base {formatCurrency(recalculatedBase)}
              {discountAmount > 0 && ` · Disc -${formatCurrency(discountAmount)}`}
              {taxRate > 0 && ` · Tax ${taxRate}%`}
              {(order.shipping.shippingAmount ?? 0) > 0 && ` · Ship ${formatCurrency(order.shipping.shippingAmount)}`}
            </span>
        ...
        <div className="flex flex-col bg-white min-h-0 overflow-hidden">
          ...
          <div className="border-b border-slate-300 px-3 py-2.5">
            <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Customer Details</div>
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <User className="h-3 w-3 text-violet-400 shrink-0" />
                <span className="text-[12px] font-semibold text-slate-800 truncate">{customerName}</span>
              </div>
              <div className="flex items-center gap-2">
                <Phone className="h-3 w-3 text-violet-400 shrink-0" />
                <span className="text-[11px] text-slate-600 truncate">{customerPhone}</span>
              </div>
              <div className="flex items-center gap-2">
                <Mail className="h-3 w-3 text-violet-400 shrink-0" />
                <span className="text-[11px] text-slate-600 truncate">{customerEmail}</span>
              </div>
              {order.discountCode && (
                <div className="flex items-center gap-2">
                  <Tag className="h-3 w-3 text-amber-400 shrink-0" />
                  <span className="text-[11px] text-amber-700 font-medium">{order.discountCode}</span>
                </div>
              )}
            </div>
          </div>
