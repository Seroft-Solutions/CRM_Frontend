...
import {
  AdvancedPagination,
  usePaginationState,
} from '@/app/(protected)/(features)/cities/components/table/advanced-pagination';
...
export function CityTable() {
  ...
  const { mutate: deleteEntity, isPending: isDeleting } = useDeleteCity({
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.filter((city) => city.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(
        ...
        (old: any[]) => old?.filter((city) => !selectedRows.has(city.id || 0))
      );
      ...
      cityToast.bulkDeleted(selectedRows.size);
      setSelectedRows(new Set());
        ...
        <AlertDialog open={showBulkDeleteDialog} onOpenChange={setShowBulkDeleteDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete the selected cities and
