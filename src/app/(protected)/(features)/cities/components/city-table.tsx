...
export function CityTable() {
  ...
  const { data: countData } = useCountCities(
    filterParams,
    {
      query: {
        enabled: true,
  ...
  const { mutate: updateEntity, isPending: isUpdating } = useUpdateCity({
    mutation: {
      onMutate: async (variables) => {
        ...
        const previousData = queryClient.getQueryData(['getAllCities', {
          page: apiPage,
          size: pageSize,
          sort: [`${sort},${order}`],
          ...filterParams,
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(['getAllCities', {
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
            ...filterParams,
        ...
        if (searchTerm) {
          queryClient.setQueryData(['searchCities', {
            query: searchTerm,
            page: apiPage,
            size: pageSize,
            sort: [`${sort},${order}`],
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        queryClient.setQueryData(['getAllCities', {
          ...
          old?.filter(city => city.id !== variables.id)
        );
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      queryClient.setQueryData(['getAllCities', {
        ...
        old?.filter(city => !selectedRows.has(city.id || 0))
      );
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              selected cities and remove their data from the server.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
