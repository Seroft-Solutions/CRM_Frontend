...
import {
  ...
  useGetAllCities,
  useSearchCities,
  useUpdateCity} from '@/core/api/generated/spring/endpoints/city-resource/city-resource.gen';
...
const COLUMN_VISIBILITY_KEY = 'city-table-columns';
...
export function CityTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'city-table-columns';
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `city-export-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const getStatusFilter = () => {
    switch (activeStatusTab) {
      case 'draft':
        return { 'status.equals': CityDTOStatus.DRAFT };
      case 'active':
        return { 'status.equals': CityDTOStatus.ACTIVE };
      case 'inactive':
        return { 'status.equals': CityDTOStatus.INACTIVE };
      case 'archived':
        return { 'status.equals': CityDTOStatus.ARCHIVED };
      case 'all':
        return {};
      default:
        return { 'status.equals': CityDTOStatus.ACTIVE };
  ...
  const { data: countData } = useCountCities(filterParams, {
    query: {
      enabled: true,
      staleTime: 0,
      refetchOnWindowFocus: true}});
    ...
    mutation: {
      onMutate: async (variables) => {
        ...
        if (previousData && Array.isArray(previousData)) {
          queryClient.setQueryData(
            ...
            (old: any[]) =>
              old.map((city) => (city.id === variables.id ? { ...city, ...variables.data } : city))
          );
        }
      ...
      onSuccess: (data, variables) => {
        queryClient.setQueryData(
          ...
          (old: any[]) => old?.map((city) => (city.id === variables.id ? data : city))
        );
  ...
  const { mutate: updateEntityStatus, isPending: isUpdatingStatus } = useUpdateCity({
    mutation: {
      onMutate: async (variables) => {
        await queryClient.cancelQueries({ queryKey: ['getAllCities'] });
        ...
        queryClient.setQueryData(
          ...
          (old: any[]) => {
            ...
            if (currentStatusFilter === newStatus || activeStatusTab === 'all') {
              ...
              return old.map((city) =>
                city.id === variables.id ? { ...city, ...variables.data } : city
              );
            } else {
              console.log(`Removing item ${variables.id} from current view`);
              return old.filter((city) => city.id !== variables.id);
            }
          }
        );
      ...
      onSuccess: (data, variables) => {
        ...
        cityToast.custom.success(`Status Updated`, `City status changed to ${statusLabel}`);
        ...
        if (currentStatusFilter !== newStatus && activeStatusTab !== 'all') {
          ...
          queryClient.setQueryData(['countCities', filterParams], (old: number) =>
            Math.max(0, (old || 0) - 1)
          );
        }
      },
  ...
  const confirmArchive = () => {
    if (archiveId) {
      ...
      if (currentEntity) {
        updateEntityStatus({
          id: archiveId,
          data: { ...currentEntity, status: CityDTOStatus.ARCHIVED }});
      }
    }
  ...
  const confirmBulkStatusChange = async () => {
    ...
    try {
      const statusValue = CityDTOStatus[bulkNewStatus as keyof typeof CityDTOStatus];
      const updatePromises = Array.from(selectedRows).map(async (id) => {
        const currentEntity = data?.find((item) => item.id === id);
        if (currentEntity) {
          return new Promise<void>((resolve, reject) => {
  ...
  const handleRelationshipUpdate = async (
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (!currentEntity) {
        ...
        reject(new Error('City not found in current data'));
        return;
      }
          ...
          onSuccess: (serverResponse) => {
            if (isBulkOperation) {
              queryClient.setQueryData(
                ...
                (old: any[]) => old?.map((city) => (city.id === entityId ? serverResponse : city))
              );
            ...
            if (!isBulkOperation) {
              cityToast.relationshipUpdated(relationshipName);
            }
            resolve();
          },
          onError: (error: any) => {
    ...
    <>
      ...
      <div className="w-full space-y-4">
                ...
                {isLoading ? (
                  ...
                  data.map((city) => (
                    <CityTableRow
                      ...
                      isSelected={selectedRows.has(city.id || 0)}
        ...
        <AlertDialog open={showBulkArchiveDialog} onOpenChange={setShowBulkArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will change the status of the selected cities to "Archived". They will no
        ...
        <AlertDialog open={showBulkStatusChangeDialog} onOpenChange={setShowBulkStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will update the status of the selected cities to "
        ...
        <AlertDialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Archive this city?</AlertDialogTitle>
              <AlertDialogDescription>
                This will change the status to "Archived". The city will no longer appear in the
        ...
        <AlertDialog open={showStatusChangeDialog} onOpenChange={setShowStatusChangeDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Change the status of this city to "
