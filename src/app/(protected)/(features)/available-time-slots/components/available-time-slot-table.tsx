...
import { AdvancedPagination, usePaginationState } from "@/app/(protected)/(features)/available-time-slots/components/table/advanced-pagination";
...
const COLUMN_VISIBILITY_KEY = 'available-time-slot-table-columns'; // v2 to force reset for auditing fields
...
export function AvailableTimeSlotTable() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const oldKey = 'available-time-slot-table-columns'; // Old key without version
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `available-time-slot-export-${new Date().toISOString().split('T')[0]}.csv`;
