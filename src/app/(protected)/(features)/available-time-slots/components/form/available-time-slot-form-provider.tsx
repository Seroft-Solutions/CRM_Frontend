...
interface AvailableTimeSlotFormProviderProps {
...
export function AvailableTimeSlotFormProvider({
  children,
  id,
  onSuccess,
  onError,
}: AvailableTimeSlotFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = availableTimeSlotFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a AvailableTimeSlotFormProvider');
  }
  return context;
}
