...
import { partyFormConfig } from "./party-form-config";
import { partyFormSchema } from "./party-form-schema";
...
interface PartyFormProviderProps {
...
export function PartyFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: PartyFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = partyFormConfig;
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a PartyFormProvider');
  }
  return context;
}
