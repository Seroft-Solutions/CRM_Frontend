...
import { PriorityStepBasic } from "./steps/priority-step-basic";
import { PriorityStepReview } from "./steps/priority-step-review";
...
export function PriorityForm({ id }: PriorityFormProps) {
  ...
  // API hooks
  const { data: existingPriority } = useGetPriority(
    { id: id || "" },
  ...
  const createPriorityMutation = useCreatePriority({
    mutation: {
      onSuccess: (data) => {
        priorityToast.created(data.data);
        router.push("/prioritys");
      },
      onError: handlePriorityError,
    },
  });
  ...
  const updatePriorityMutation = useUpdatePriority({
    mutation: {
      onSuccess: (data) => {
        priorityToast.updated(data.data);
        router.push("/prioritys");
      },
      onError: handlePriorityError,
    },
  ...
  // Load existing data
  if (existingPriority && !form.formState.isDirty) {
    const data = existingPriority.data;
    if (data) {
      const formData: any = {};
  ...
  // Form submission
  const onSubmit = (values: z.infer<typeof prioritySchema>) => {
    ...
    const priorityData: PriorityDTO = {
      name: values.name,
    ...
    if (isNew) {
      createPriorityMutation.mutate({ data: priorityData });
    } else {
      updatePriorityMutation.mutate({
        id: id!,
        data: { ...existingPriority?.data, ...priorityData },
      });
    }
  };

  const isLoading = createPriorityMutation.isPending || updatePriorityMutation.isPending;
          ...
          <Card>
            <CardContent className="p-4 sm:p-6">
              ...
              {STEPS[currentStep].id === 'basic' && (
                <PriorityStepBasic form={form} />
              )}

              {STEPS[currentStep].id === 'review' && (
                <PriorityStepReview form={form} />
              )}
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:justify-between">
            ...
            {STEPS[currentStep].id === 'review' && confirmSubmission ? (
              ...
              >
                ...
                {isLoading ? "Saving..." : `${isNew ? "Create" : "Update"} Priority`}
              </Button>
            ) : STEPS[currentStep].id === 'review' ? (
              <Button
                type="submit"
