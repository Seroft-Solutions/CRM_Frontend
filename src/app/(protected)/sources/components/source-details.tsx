...
export function SourceDetails({ id }: SourceDetailsProps) {
  const router = useRouter();
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  // Fetch entity details
  const { data: entity, isLoading } = useGetSource(id, {
    query: {
      enabled: !!id,
    },
  });
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardDescription>
            Viewing details for Source #id{entity.id}
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="flex justify-end gap-3 mt-8 pt-6 border-t">
            <Button variant="outline" asChild>
              <Link href={`/sources/${id}/edit`} className="flex items-center gap-2">
