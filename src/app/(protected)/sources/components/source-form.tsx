...
import { SourceStepBasic } from "./steps/source-step-basic";
import { SourceStepReview } from "./steps/source-step-review";
...
export function SourceForm({ id }: SourceFormProps) {
  ...
  // API hooks
  const { data: existingSource } = useGetSource(
    { id: id || "" },
  ...
  // Load existing data
  if (existingSource && !form.formState.isDirty) {
  ...
  const onSubmit = (values: z.infer<typeof sourceSchema>) => {
    ...
    if (isNew) {
      createSourceMutation.mutate({ data: sourceData });
    } else {
      updateSourceMutation.mutate({
        id: id!,
        data: { ...existingSource?.data, ...sourceData },
      });
    }
  };
          ...
          <Card>
            <CardContent className="p-4 sm:p-6">
              ...
              {STEPS[currentStep].id === 'basic' && (
                <SourceStepBasic form={form} />
              )}

              {STEPS[currentStep].id === 'review' && (
                <SourceStepReview form={form} />
              )}
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:justify-between">
            ...
            {STEPS[currentStep].id === 'review' && confirmSubmission ? (
              ...
              >
                ...
                {isLoading ? "Saving..." : `${isNew ? "Create" : "Update"} Source`}
              </Button>
            ) : STEPS[currentStep].id === 'review' ? (
              <Button
                type="submit"
