...
export function ChannelTypeForm({ id }: ChannelTypeFormProps) {
  ...
  const [formSessionId] = useState(() => {
    ...
    const existingSession = sessionStorage.getItem('ChannelType_FormSession');
    if (existingSession && isNew) {
      return existingSession;
    }
    const newSessionId = `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    if (isNew) {
      sessionStorage.setItem('ChannelType_FormSession', newSessionId);
    }
    return newSessionId;
  });
  ...
  const saveFormState = React.useCallback(() => {
    ...
    const storageKey = `ChannelTypeFormState_${formSessionId}`;
  ...
  const restoreFormState = React.useCallback(() => {
    ...
    if (entityCreationContext) {
      try {
        ...
        if (context.sourceEntity && context.sourceEntity !== 'ChannelType') {
    ...
    const currentSessionId = sessionStorage.getItem('ChannelType_FormSession');
    ...
    if (savedStateStr) {
      try {
        ...
        const isSameEntity = savedState.entity === 'ChannelType';
  ...
  const clearOldFormStates = React.useCallback(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith('ChannelTypeFormState_') && !key.endsWith(formSessionId)) {
    ...
    const entityToSave = {
      ...
      ...(entity && !isNew ? {
        ...Object.keys(entity).reduce((acc, key) => {
          const isFormField = ['name','description','commissionRate',].includes(key);
  ...
  const cleanupFormState = React.useCallback(() => {
    const storageKey = `ChannelTypeFormState_${formSessionId}`;
    localStorage.removeItem(storageKey);
    sessionStorage.removeItem('ChannelType_FormSession');
    
    // Clear all old form states for this entity type
    const keysToRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key?.startsWith('ChannelTypeFormState_')) {
        keysToRemove.push(key);
      }
    }
    keysToRemove.forEach(key => localStorage.removeItem(key));
  ...
  const validateStep = async () => {
    ...
    switch (currentStepId) {
      case 'basic':
        fieldsToValidate = ['name','description','commissionRate',];
        break;
      case 'dates':
        fieldsToValidate = [];
        break;
            ...
            {currentStep === STEPS.length - 1 ? (
              !confirmSubmission ? (
                ...
                >
                  ...
                  {isCreating || isUpdating ? "Submitting..." : `${isNew ? "Create" : "Update"} Channel Type`}
                </Button>
              )
            ) : (
              <Button
