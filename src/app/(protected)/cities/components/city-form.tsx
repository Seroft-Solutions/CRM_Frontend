...
export function CityForm({ id }: CityFormProps) {
  ...
  const [formSessionId] = useState(() => {
    ...
    const existingSession = sessionStorage.getItem('City_FormSession');
    if (existingSession && isNew) {
      return existingSession;
    }
    const newSessionId = `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    if (isNew) {
      sessionStorage.setItem('City_FormSession', newSessionId);
    }
    return newSessionId;
  });
  ...
  const saveFormState = React.useCallback(() => {
    ...
    const storageKey = `CityFormState_${formSessionId}`;
  ...
  const restoreFormState = React.useCallback(() => {
    ...
    if (entityCreationContext) {
      try {
        ...
        if (context.sourceEntity && context.sourceEntity !== 'City') {
    ...
    const currentSessionId = sessionStorage.getItem('City_FormSession');
    ...
    if (savedStateStr) {
      try {
        ...
        const isSameEntity = savedState.entity === 'City';
  ...
  const clearOldFormStates = React.useCallback(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith('CityFormState_') && !key.endsWith(formSessionId)) {
  ...
  const cleanupFormState = React.useCallback(() => {
    const storageKey = `CityFormState_${formSessionId}`;
    localStorage.removeItem(storageKey);
    sessionStorage.removeItem('City_FormSession');
    
    // Clear all old form states for this entity type
    const keysToRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key?.startsWith('CityFormState_')) {
        keysToRemove.push(key);
      }
    }
    keysToRemove.forEach(key => localStorage.removeItem(key));
