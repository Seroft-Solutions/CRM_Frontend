  ...
  return (
    <TableRow>
      <TableCell className="w-12 px-3 py-2">
        <Checkbox
          checked={isSelected}
          onCheckedChange={() => meeting.id && onSelect(meeting.id)}
        />
      </TableCell>
      ...
      <TableCell className="whitespace-nowrap px-1 py-2">
        <RelationshipCell
          ...
          options={relationshipConfigs.find(config => config.name === "assignedParty")?.options || []}
          displayField="name"
          onUpdate={onRelationshipUpdate || (() => Promise.resolve())}
          isEditable={relationshipConfigs.find(config => config.name === "assignedParty")?.isEditable || false}
          isLoading={isUpdating}
          className="min-w-[150px]"
