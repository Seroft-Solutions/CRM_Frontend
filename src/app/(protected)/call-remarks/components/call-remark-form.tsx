...
import { CallRemarkStepBasic } from "./steps/call-remark-step-basic";
import { CallRemarkStepDates } from "./steps/call-remark-step-dates";
import { CallRemarkStepReview } from "./steps/call-remark-step-review";
...
export function CallRemarkForm({ id }: CallRemarkFormProps) {
  ...
  // Form setup
  const form = useForm<z.infer<typeof callRemarkSchema>>({
    resolver: zodResolver(callRemarkSchema),
    defaultValues: {
      remark: "",
      dateTime: "",
      call: undefined,
  ...
  // API hooks
  const { data: existingCallRemark } = useGetCallRemark(
    { id: id || "" },
    { query: { enabled: !isNew && !!id } }
  );

  const createCallRemarkMutation = useCreateCallRemark({
    mutation: {
      onSuccess: (data) => {
        callRemarkToast.created(data.data);
        router.push("/call-remarks");
      },
      onError: handleCallRemarkError,
    },
  });

  const updateCallRemarkMutation = useUpdateCallRemark({
    mutation: {
      onSuccess: (data) => {
        callRemarkToast.updated(data.data);
        router.push("/call-remarks");
      },
      onError: handleCallRemarkError,
    },
  ...
  // Load existing data
  if (existingCallRemark && !form.formState.isDirty) {
    const data = existingCallRemark.data;
    if (data) {
      const formData: any = {};
      ...
      if (data.dateTime) {
        formData.dateTime = new Date(data.dateTime);
      }
      if (data.call) {
  ...
  // Form submission
  const onSubmit = (values: z.infer<typeof callRemarkSchema>) => {
    ...
    if (isNew) {
      createCallRemarkMutation.mutate({ data: callRemarkData });
    } else {
      updateCallRemarkMutation.mutate({
        id: id!,
        data: { ...existingCallRemark?.data, ...callRemarkData },
      });
    }
  };

  const isLoading = createCallRemarkMutation.isPending || updateCallRemarkMutation.isPending;
          ...
          <Card>
            <CardContent className="p-4 sm:p-6">
              ...
              {STEPS[currentStep].id === 'basic' && (
                <CallRemarkStepBasic form={form} />
              )}

              {STEPS[currentStep].id === 'dates' && (
                <CallRemarkStepDates form={form} handleEntityCreated={handleEntityCreated} />
              )}

              {STEPS[currentStep].id === 'review' && (
                <CallRemarkStepReview form={form} />
              )}
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:justify-between">
            ...
            {STEPS[currentStep].id === 'review' && confirmSubmission ? (
              ...
              >
                ...
                {isLoading ? "Saving..." : `${isNew ? "Create" : "Update"} CallRemark`}
              </Button>
            ) : STEPS[currentStep].id === 'review' ? (
              <Button
                type="submit"
