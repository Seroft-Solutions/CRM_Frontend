...
export function CallRemarkForm({ id }: CallRemarkFormProps) {
  ...
  const { mutate: createEntity, isPending: isCreating } = useCreateCallRemark({
    mutation: {
      onSuccess: (data) => {
        ...
        localStorage.removeItem('CallRemarkFormState');
  ...
  const saveFormState = React.useCallback(() => {
    ...
    localStorage.setItem('CallRemarkFormState', JSON.stringify(formState));
    console.log('Form state saved:', formState);
  }, [form, currentStep]);

  const restoreFormState = React.useCallback(() => {
    const savedStateStr = localStorage.getItem('CallRemarkFormState');
    if (savedStateStr) {
      try {
        const savedState = JSON.parse(savedStateStr);
        const isRecent = Date.now() - savedState.timestamp < 30 * 60 * 1000; // 30 minutes
        
        if (isRecent && savedState.entity === 'CallRemark') {
          setIsRestoring(true);
