...
export function RoleForm({ id }: RoleFormProps) {
  ...
  const [formSessionId] = useState(() => {
    ...
    const existingSession = sessionStorage.getItem('Role_FormSession');
    if (existingSession && isNew) {
      return existingSession;
    }
    const newSessionId = `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    if (isNew) {
      sessionStorage.setItem('Role_FormSession', newSessionId);
    }
    return newSessionId;
  });
  ...
  const saveFormState = React.useCallback(() => {
    ...
    const storageKey = `RoleFormState_${formSessionId}`;
  ...
  const restoreFormState = React.useCallback(() => {
    ...
    if (entityCreationContext) {
      try {
        ...
        if (context.sourceEntity && context.sourceEntity !== 'Role') {
    ...
    const currentSessionId = sessionStorage.getItem('Role_FormSession');
    ...
    if (savedStateStr) {
      try {
        ...
        const isSameEntity = savedState.entity === 'Role';
  ...
  const clearOldFormStates = React.useCallback(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith('RoleFormState_') && !key.endsWith(formSessionId)) {
  ...
  useEffect(() => {
    if (entity && !isRestoring) {
      const formValues = {
        ...
        users: entity.users?.map(item => item.id),
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    const entityToSave = {
      ...
      users: data.users?.map(id => ({ id: id })),

      ...(entity && !isNew ? {
        ...Object.keys(entity).reduce((acc, key) => {
          const isFormField = ['name','description','users',].includes(key);
  ...
  const cleanupFormState = React.useCallback(() => {
    const storageKey = `RoleFormState_${formSessionId}`;
    localStorage.removeItem(storageKey);
    sessionStorage.removeItem('Role_FormSession');
    
    // Clear all old form states for this entity type
    const keysToRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key?.startsWith('RoleFormState_')) {
        keysToRemove.push(key);
      }
    }
    keysToRemove.forEach(key => localStorage.removeItem(key));
                      ...
                      <div className="space-y-1">
                        ...
                        <dd className="text-sm">
                          <Badge variant="outline">
                            {Array.isArray(form.watch('users')) ? 
                              `${form.watch('users').length} selected` : 'None selected'}
                          </Badge>
                        </dd>
                      </div>
                    </div>
            ...
            {currentStep === STEPS.length - 1 ? (
              !confirmSubmission ? (
                ...
                >
                  ...
                  {isCreating || isUpdating ? "Submitting..." : `${isNew ? "Create" : "Update"} Role`}
                </Button>
              )
            ) : (
              <Button
