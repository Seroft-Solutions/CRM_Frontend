...
interface UserAvailabilityFormProviderProps {
...
export function UserAvailabilityFormProvider({ 
  children, 
  id, 
  onSuccess, 
  onError 
}: UserAvailabilityFormProviderProps) {
  const router = useRouter();
  const isNew = !id;
  const config = userAvailabilityFormConfig;
  ...
  const form = useForm({
    resolver: zodResolver(userAvailabilityFormSchema),
...
export function useEntityForm(): FormContextValue {
  ...
  if (!context) {
    throw new Error('useEntityForm must be used within a UserAvailabilityFormProvider');
  }
  return context;
}
