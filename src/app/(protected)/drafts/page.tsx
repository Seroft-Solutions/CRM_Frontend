...
import { useGetAllUserDrafts, useDeleteUserDraft } from "@/core/api/generated/spring/endpoints/user-draft-resource/user-draft-resource.gen";
...
export default function DraftsManagementPage() {
  ...
  const filteredDrafts = allDrafts.filter(draft => {
    const matchesSearch = draft.data.entityType.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         draft.id.toString().includes(searchQuery);
    const matchesEntityType = selectedEntityType === "all" || draft.entityType === selectedEntityType;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage your saved form drafts across all entities
          </p>
        </div>
      ...
      {isLoadingAny ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {sortedDrafts.map((draft) => (
            <Card key={`${draft.entityType}-${draft.id}`} className="hover:shadow-md transition-shadow">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg">
                    Draft #{draft.id}
                  </CardTitle>
                  <Badge className={getEntityColor(draft.entityType)}>
                    {draft.entityType}
                  </Badge>
                </div>
              </CardHeader>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this {draftToDelete?.entityType} draft? 
