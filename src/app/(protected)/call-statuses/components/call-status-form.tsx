...
export function CallStatusForm({ id }: CallStatusFormProps) {
  ...
  const saveFormState = React.useCallback(() => {
    ...
    localStorage.setItem('CallStatusFormState', JSON.stringify(formState));
    console.log('Form state saved:', formState);
  }, [form, currentStep]);

  const restoreFormState = React.useCallback(() => {
    const savedStateStr = localStorage.getItem('CallStatusFormState');
    if (savedStateStr) {
      try {
        const savedState = JSON.parse(savedStateStr);
        const isRecent = Date.now() - savedState.timestamp < 30 * 60 * 1000; // 30 minutes
        
        if (isRecent && savedState.entity === 'CallStatus') {
          setIsRestoring(true);
