...
export function CallStatusForm({ id }: CallStatusFormProps) {
  ...
  const [formSessionId] = useState(() => {
    ...
    const existingSession = sessionStorage.getItem('CallStatus_FormSession');
    if (existingSession && isNew) {
      return existingSession;
    }
    const newSessionId = `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    if (isNew) {
      sessionStorage.setItem('CallStatus_FormSession', newSessionId);
    }
    return newSessionId;
  });
  ...
  const saveFormState = React.useCallback(() => {
    ...
    const storageKey = `CallStatusFormState_${formSessionId}`;
  ...
  const restoreFormState = React.useCallback(() => {
    ...
    if (entityCreationContext) {
      try {
        ...
        if (context.sourceEntity && context.sourceEntity !== 'CallStatus') {
    ...
    const currentSessionId = sessionStorage.getItem('CallStatus_FormSession');
    ...
    if (savedStateStr) {
      try {
        ...
        const isSameEntity = savedState.entity === 'CallStatus';
  ...
  const clearOldFormStates = React.useCallback(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith('CallStatusFormState_') && !key.endsWith(formSessionId)) {
  ...
  const cleanupFormState = React.useCallback(() => {
    const storageKey = `CallStatusFormState_${formSessionId}`;
    localStorage.removeItem(storageKey);
    sessionStorage.removeItem('CallStatus_FormSession');
    
    // Clear all old form states for this entity type
    const keysToRemove: string[] = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key?.startsWith('CallStatusFormState_')) {
        keysToRemove.push(key);
      }
    }
    keysToRemove.forEach(key => localStorage.removeItem(key));
            ...
            {currentStep === STEPS.length - 1 ? (
              !confirmSubmission ? (
                ...
                >
                  ...
                  {isCreating || isUpdating ? "Submitting..." : `${isNew ? "Create" : "Update"} Call Status`}
                </Button>
              )
            ) : (
              <Button
