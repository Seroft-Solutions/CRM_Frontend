...
async function handleAssignGroups(data: {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      synced: result.sync.synced,
      message: result.sync.synced 
        ? `Groups ${data.action}ed and synced successfully`
        : `Groups ${data.action}ed but sync pending`,
    });
  } catch (error: any) {
    return NextResponse.json(
      { 
...
async function handleAssignRoles(data: {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      synced: result.sync.synced,
      message: result.sync.synced 
        ? `Roles ${data.action}ed and synced successfully`
        : `Roles ${data.action}ed but sync pending`,
    });
  } catch (error: any) {
    return NextResponse.json(
      { 
