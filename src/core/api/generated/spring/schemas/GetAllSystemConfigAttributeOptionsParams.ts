/**
 * Generated by orval v7.8.0 🍺
 * Do not edit manually.
 * Crm Backend API
 * Crm Backend API documentation
 * OpenAPI spec version: 0.0.1
 */
import type { GetAllSystemConfigAttributeOptionsStatusEquals } from './GetAllSystemConfigAttributeOptionsStatusEquals';
import type { GetAllSystemConfigAttributeOptionsStatusNotEquals } from './GetAllSystemConfigAttributeOptionsStatusNotEquals';
import type { GetAllSystemConfigAttributeOptionsStatusInItem } from './GetAllSystemConfigAttributeOptionsStatusInItem';
import type { GetAllSystemConfigAttributeOptionsStatusNotInItem } from './GetAllSystemConfigAttributeOptionsStatusNotInItem';

export type GetAllSystemConfigAttributeOptionsParams = {
'id.greaterThan'?: number;
'id.lessThan'?: number;
'id.greaterThanOrEqual'?: number;
'id.lessThanOrEqual'?: number;
'id.equals'?: number;
'id.notEquals'?: number;
'id.specified'?: boolean;
'id.in'?: number[];
'id.notIn'?: number[];
'code.contains'?: string;
'code.doesNotContain'?: string;
'code.equals'?: string;
'code.notEquals'?: string;
'code.specified'?: boolean;
'code.in'?: string[];
'code.notIn'?: string[];
'label.contains'?: string;
'label.doesNotContain'?: string;
'label.equals'?: string;
'label.notEquals'?: string;
'label.specified'?: boolean;
'label.in'?: string[];
'label.notIn'?: string[];
'sortOrder.greaterThan'?: number;
'sortOrder.lessThan'?: number;
'sortOrder.greaterThanOrEqual'?: number;
'sortOrder.lessThanOrEqual'?: number;
'sortOrder.equals'?: number;
'sortOrder.notEquals'?: number;
'sortOrder.specified'?: boolean;
'sortOrder.in'?: number[];
'sortOrder.notIn'?: number[];
'status.equals'?: GetAllSystemConfigAttributeOptionsStatusEquals;
'status.notEquals'?: GetAllSystemConfigAttributeOptionsStatusNotEquals;
'status.specified'?: boolean;
'status.in'?: GetAllSystemConfigAttributeOptionsStatusInItem[];
'status.notIn'?: GetAllSystemConfigAttributeOptionsStatusNotInItem[];
'createdBy.contains'?: string;
'createdBy.doesNotContain'?: string;
'createdBy.equals'?: string;
'createdBy.notEquals'?: string;
'createdBy.specified'?: boolean;
'createdBy.in'?: string[];
'createdBy.notIn'?: string[];
'createdDate.greaterThan'?: string;
'createdDate.lessThan'?: string;
'createdDate.greaterThanOrEqual'?: string;
'createdDate.lessThanOrEqual'?: string;
'createdDate.equals'?: string;
'createdDate.notEquals'?: string;
'createdDate.specified'?: boolean;
'createdDate.in'?: string[];
'createdDate.notIn'?: string[];
'lastModifiedBy.contains'?: string;
'lastModifiedBy.doesNotContain'?: string;
'lastModifiedBy.equals'?: string;
'lastModifiedBy.notEquals'?: string;
'lastModifiedBy.specified'?: boolean;
'lastModifiedBy.in'?: string[];
'lastModifiedBy.notIn'?: string[];
'lastModifiedDate.greaterThan'?: string;
'lastModifiedDate.lessThan'?: string;
'lastModifiedDate.greaterThanOrEqual'?: string;
'lastModifiedDate.lessThanOrEqual'?: string;
'lastModifiedDate.equals'?: string;
'lastModifiedDate.notEquals'?: string;
'lastModifiedDate.specified'?: boolean;
'lastModifiedDate.in'?: string[];
'lastModifiedDate.notIn'?: string[];
'attributeId.greaterThan'?: number;
'attributeId.lessThan'?: number;
'attributeId.greaterThanOrEqual'?: number;
'attributeId.lessThanOrEqual'?: number;
'attributeId.equals'?: number;
'attributeId.notEquals'?: number;
'attributeId.specified'?: boolean;
'attributeId.in'?: number[];
'attributeId.notIn'?: number[];
distinct?: boolean;
/**
 * Zero-based page index (0..N)
 */
page?: number;
/**
 * The size of the page to be returned
 */
size?: number;
/**
 * Sorting criteria in the format: property,(asc|desc). Default sort order is ascending. Multiple sort criteria are supported.
 */
sort?: string[];
};
