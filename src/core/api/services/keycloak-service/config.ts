...
export const KEYCLOAK_SERVICE_CONFIG: BaseServiceConfig = {
  baseURL: process.env.AUTH_KEYCLOAK_ISSUER?.replace('/realms/crm', '') || 'http://localhost:9090',
  timeout: 30000,
  authType: 'bearer',
  headers: {
...
export const KEYCLOAK_REALM = (() => {
  const issuer = process.env.AUTH_KEYCLOAK_ISSUER || 'http://localhost:9090/realms/crm';
