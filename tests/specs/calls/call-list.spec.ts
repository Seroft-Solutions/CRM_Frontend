  ...
  test.describe('Call List Page', () => {
    ...
    test('should handle call status updates', async ({ page }) => {
      ...
      const statusCell = firstRow
        .locator('td')
        .filter({ hasText: /pending|in progress|completed/i });
    ...
    test('should display call remarks section when available', async ({ page }) => {
      ...
      const remarksSection = page
        .locator('[data-testid="call-remarks"], .call-remarks, td')
        .filter({ hasText: /remark/i });
      ...
      if (await remarksSection.first().isVisible()) {
        ...
        const remarksOnDetailPage = page
          .locator('h2, h3, .section-title')
          .filter({ hasText: /remark/i });
